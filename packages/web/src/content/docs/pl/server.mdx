---
title: Serwer
description: Interakcja z serwerem alphacode za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `alphacode serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient alphacode.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `alphacode.local` |
...
Ustaw `ALPHACODE_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `alphacode` lub ustaw `ALPHACODE_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `alphacode serve`, jak i `alphacode web`.
...
Kiedy uruchomisz `alphacode`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera alphacode do programowej interakcji z alphacode.
:::

Ta architektura umożliwia obsługę wielu klientów przez alphacode i programową interakcję z kodem otwartym.

Możesz uruchomić `alphacode serve`, aby uruchomić samodzielny serwer. Jeśli masz
alphacode TUI działa, `alphacode serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki alphacode [IDE](/docs/ide).
...
Serwer alphacode udostępnia następujące interfejsy API.
