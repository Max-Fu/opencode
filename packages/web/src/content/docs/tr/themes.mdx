...
alphacode ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak alphacode kendi `alphacode` temasını kullanır.
...
alphacode birden fazla yerleşik temayla gelir.
...
- alphacode'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
alphacode, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/alphacode/themes/*.json` veya `$XDG_CONFIG_HOME/alphacode/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.alphacode/themes/*.json`
4. **Geçerli çalışma dizini** - `./.alphacode/themes/*.json`
