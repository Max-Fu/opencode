  ...
  test("moves an existing canonical web bucket into local scope", () => {
    expect(
      migrateCanonicalLocalServerState(
        {
          list: [],
          projects: { "https://alphacode.example.com": [{ worktree: "/remote", expanded: true }] },
          lastProject: { "https://alphacode.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://alphacode.example.com"),
      ),
    ).toEqual({
      list: [],
      ...
      migrateCanonicalLocalServerState(
        {
          projects: {
            ...
            "https://alphacode.example.com": [
              { worktree: "/local", expanded: true },
              { worktree: "/remote", expanded: true },
            ],
          },
          lastProject: { local: "/local", "https://alphacode.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://alphacode.example.com"),
      ),
    ).toEqual({
