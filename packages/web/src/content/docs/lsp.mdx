---
title: LSP Servers
description: AlphaCode integrates with your LSP servers.
---

AlphaCode can integrate with Language Server Protocol (LSP) servers to use diagnostics as feedback for the agent.

---

## Built-in

AlphaCode comes with several built-in LSP servers for popular languages:
...
:::note
You can disable automatic LSP server downloads by setting the `ALPHACODE_DISABLE_LSP_DOWNLOAD` environment variable to `true`.
:::
...
When LSP is enabled and alphacode opens a file, it:
...
You can enable and customize LSP servers through the `lsp` section in your alphacode config.
...
```json title="alphacode.json" {5-13}
{
  "$schema": "https://alphacode.ai/config.json",
  "lsp": {
    "custom-lsp": {
