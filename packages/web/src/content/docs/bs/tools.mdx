...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. AlphaCode dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u AlphaCode.
...
:::note
Ovaj alat je dostupan samo kada je `ALPHACODE_EXPERIMENTAL_LSP_TOOL=true` (ili `ALPHACODE_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz AlphaCode provajdera ili kada je varijabla `ALPHACODE_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju AlphaCode:
