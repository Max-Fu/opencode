...
Puoi aggiungere strumenti esterni a AlphaCode usando il _Model Context Protocol_, o MCP. AlphaCode supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [AlphaCode Config](https://alphacode.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/alphacode`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
AlphaCode gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, AlphaCode:
...
Se il server richiede autenticazione, AlphaCode ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `alphacode mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, AlphaCode memorizzera' i token in modo sicuro in `~/.local/share/alphacode/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in AlphaCode insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione AlphaCode come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare AlphaCode al tuo account Sentry.
