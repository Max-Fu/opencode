---
title: Servidor
description: Interaja com o servidor alphacode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `alphacode serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente alphacode pode usar.
...
```bash
alphacode serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `alphacode.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
alphacode serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `ALPHACODE_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `alphacode`, ou defina `ALPHACODE_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `alphacode serve` quanto ao `alphacode web`.

```bash
ALPHACODE_SERVER_PASSWORD=your-password alphacode serve
```
...
Quando você executa `alphacode`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor alphacode para interagir com o alphacode programaticamente.
:::

Essa arquitetura permite que o alphacode suporte múltiplos clientes e permite que você interaja com o alphacode programaticamente.

Você pode executar `alphacode serve` para iniciar um servidor autônomo. Se você tiver o TUI do alphacode em execução, `alphacode serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do alphacode [IDE](/docs/ide).
...
O servidor alphacode expõe as seguintes APIs.
