...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g alphacode-ai",
      pnpm: "pnpm uninstall -g alphacode-ai",
      bun: "bun remove -g alphacode-ai",
      yarn: "yarn global remove alphacode-ai",
      brew: "brew uninstall alphacode",
      choco: "choco uninstall alphacode",
      scoop: "scoop uninstall alphacode",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "alphacode", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# alphacode") || content.includes(".alphacode/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".alphacode/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".alphacode/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".alphacode"))
    ) {
      continue
    }
