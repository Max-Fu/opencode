import type { WslDistroProbe, WslAlphacodeCheck, WslServerItem } from "../../preload/types"
...
export function clearWslDistroState(
  ...
  alphacodeChecks: Record<string, WslAlphacodeCheck>,
  distro: string,
) {
  const nextDistroProbes = { ...distroProbes }
  const nextAlphacodeChecks = { ...alphacodeChecks }
  delete nextDistroProbes[distro]
  delete nextAlphacodeChecks[distro]
  return { distroProbes: nextDistroProbes, alphacodeChecks: nextAlphacodeChecks }
}
