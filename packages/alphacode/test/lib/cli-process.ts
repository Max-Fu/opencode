// Subprocess test harness for the alphacode CLI. Spawns the real binary against
...
//
// Configuration flows through alphacode's built-in test affordances:
//   - ALPHACODE_CONFIG_CONTENT      : provider config inline, no files to find
//   - ALPHACODE_TEST_HOME           : pins os.homedir() → tmpdir
//   - ALPHACODE_DISABLE_PROJECT_CONFIG : skip walking up for alphacode.json
//   - ALPHACODE_PURE                : skip external plugin discovery + install
//   - ALPHACODE_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `alphacode.run` is fully wired. The shape supports adding more
// builders (`alphacode.serve(opts)`, `alphacode.acp(opts)`, `alphacode.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of AlphacodeCli.
...
import { AppProcess } from "@alphacode-ai/core/process"
...
const alphacodeRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(alphacodeRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    ALPHACODE_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    ALPHACODE_CONFIG_CONTENT: configJson,
    ALPHACODE_DISABLE_PROJECT_CONFIG: "1",
    ALPHACODE_PURE: "1",
    ALPHACODE_DISABLE_AUTOUPDATE: "1",
    ALPHACODE_DISABLE_AUTOCOMPACT: "1",
    ALPHACODE_DISABLE_MODELS_FETCH: "1",
    ALPHACODE_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `alphacode run`. New flags should
...
// `alphacode serve` is a long-lived process — it never exits on its own.
...
// `alphacode acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type AlphacodeCli = {
  ...
  // Spawn `alphacode serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `alphacode acp` and return a duplex JSON-RPC handle. Long-lived:
...
export type CliFixture = {
  ...
  readonly alphacode: AlphacodeCli
}
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("alphacode.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          ALPHACODE_CONFIG_CONTENT: JSON.stringify({
    ...
    const startRun = Effect.fn("alphacode.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("alphacode.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "alphacode server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `alphacode serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("alphacode.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const alphacode: AlphacodeCli = { run, startRun, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, alphacode })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `alphacode.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
      Layer.mergeAll(
        TestLLMServer.layer,
...
function expectExit(result: RunResult, expected: number, label = "alphacode") {
...
// every call site. Use this for any test that needs the alphacode CLI fixture.
//
...
// resources (e.g. `alphacode.serve`) without an extra `Effect.scoped` wrapper —
