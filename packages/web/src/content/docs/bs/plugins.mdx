---
title: Dodaci
description: Napišite vlastite dodatke za proširenje AlphaCode.
---

Dodaci vam omogućavaju da proširite AlphaCode spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja AlphaCode.
...
- `.alphacode/plugins/` - Dodaci na nivou projekta
- `~/.config/alphacode/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/alphacode/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/alphacode/alphacode.json`)
2. Konfiguracija projekta (`alphacode.json`)
3. Globalni direktorij dodataka (`~/.config/alphacode/plugins/`)
4. Direktorij dodataka projekta (`.alphacode/plugins/`)
...
AlphaCode pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: Alphacode SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje AlphaCode.
...
Spriječite alphacode da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u alphacode:
...
Pomoćnik `tool` kreira prilagođeni alat koji alphacode može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za alphacode zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://alphacode.ai/docs/sdk) za detalje.
