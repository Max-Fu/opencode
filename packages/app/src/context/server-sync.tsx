...
import { NormalizedProviderListResponse } from "@alphacode-ai/session-ui/context"
...
export const loadLspQuery = (scope: ServerScope, directory: string, sdk: AlphacodeClient) =>
  queryOptions({
...
function makeQueryOptionsApi(
  scope: ServerScope,
  serverSDK: () => AlphacodeClient,
  serverAPI: ServerApi,
  sdkFor: (dir: PathKey) => AlphacodeClient,
  protocol: Promise<"v1" | "v2">,
) {
  return {
...
export function createServerSyncContextInner(serverSDK: ServerSDK) {
  ...
  const sdkCache = new Map<string, AlphacodeClient>()
