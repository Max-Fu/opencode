...
Las herramientas permiten que LLM realice acciones en su código base. AlphaCode viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en AlphaCode.
...
:::note
Esta herramienta solo está disponible cuando `ALPHACODE_EXPERIMENTAL_LSP_TOOL=true` (o `ALPHACODE_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor AlphaCode o cuando la variable de entorno `ALPHACODE_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar AlphaCode:

```bash
ALPHACODE_ENABLE_EXA=1 alphacode
```
