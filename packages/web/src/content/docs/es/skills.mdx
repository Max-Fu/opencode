...
Las habilidades del agente permiten a AlphaCode descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
AlphaCode busca estas ubicaciones:

- Configuración del proyecto: `.alphacode/skills/<name>/SKILL.md`
- Configuración global: `~/.config/alphacode/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, AlphaCode sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.alphacode/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/alphacode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.alphacode/skills/git-release/SKILL.md` así:
...
AlphaCode enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `alphacode.json`:
...
**Para agentes integrados** (en `alphacode.json`):
