...
AlphaCode Go ist ein kostengünstiges Abonnement — **5 $ für deinen ersten Monat**, danach **10 $/Monat** —, das dir zuverlässigen Zugriff auf beliebte offene Coding-Modelle bietet.

Go funktioniert wie jeder andere Provider in AlphaCode. Du abonnierst AlphaCode Go und
erhältst deinen API-Key. Es ist **völlig optional** und du musst es nicht nutzen, um
AlphaCode zu verwenden.
...
AlphaCode Go bietet dir Zugriff auf diese Modelle für **5 $ im ersten Monat**, danach **10 $/Monat**.

---

## Wie es funktioniert

AlphaCode Go funktioniert wie jeder andere Provider in AlphaCode.

1. Du meldest dich bei **<a href={console}>AlphaCode Zen</a>** an, abonnierst Go und kopierst deinen API-Key.
2. Du führst den Befehl `/connect` in der TUI aus, wählst `AlphaCode Go` und fügst deinen API-Key ein.
3. Führe `/models` in der TUI aus, um die Liste der über Go verfügbaren Modelle zu sehen.

:::note
Nur ein Mitglied pro Workspace kann AlphaCode Go abonnieren.
:::
...
AlphaCode Go beinhaltet die folgenden Limits:
...
Die [Modell-ID](/docs/config/#models) in deiner AlphaCode Config verwendet das Format `alphacode-go/<model-id>`. Für Kimi K3 würdest du beispielsweise `alphacode-go/kimi-k3` in deiner Config verwenden.
...
Wir haben AlphaCode Go entwickelt, um:
...
4. **Keinen Lock-in-Effekt** zu haben, indem wir dir ermöglichen, auch jeden anderen Provider mit AlphaCode zu nutzen.
