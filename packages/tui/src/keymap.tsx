...
export const ALPHACODE_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const ALPHACODE_MODE_KEY = "alphacode.mode"

export const AlphacodeKeymapProvider = KeymapProvider
export const useAlphacodeKeymap = useKeymap

export { useBindings, useKeymapSelector }

export type OpenTuiKeymap = ReturnType<typeof useKeymap>
type AlphacodeModeStack = ReturnType<typeof createAlphacodeModeStack>
...
const modeStacks = new WeakMap<OpenTuiKeymap, AlphacodeModeStack>()

function isVisiblePaletteCommand(command: Command) {
  return command.hidden !== true && command.name !== COMMAND_PALETTE_COMMAND
}

export function createAlphacodeModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(ALPHACODE_MODE_KEY, ALPHACODE_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(ALPHACODE_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(ALPHACODE_MODE_KEY, stack.at(-1)?.mode ?? ALPHACODE_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? ALPHACODE_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(ALPHACODE_MODE_KEY, undefined)
...
export function useAlphacodeModeStack() {
  return getAlphacodeModeStack(useAlphacodeKeymap())
}

export function getAlphacodeModeStack(keymap: OpenTuiKeymap) {
...
export function registerAlphacodeKeymap(keymap: OpenTuiKeymap, renderer: CliRenderer, config: ResolvedKeymapConfig) {
  const modeStack = createAlphacodeModeStack(keymap)
...
export function useCommandSlashes(): Accessor<readonly CommandSlashEntry[]> {
  const keymap = useAlphacodeKeymap()
