...
Practical reference for a future `packages/server` split after the alphacode
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/alphacode`.
...
Until enough shared service code lives outside `packages/alphacode`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/alphacode`

It should not import `packages/alphacode` services while `packages/alphacode`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on alphacode-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/alphacode` implementation details.
