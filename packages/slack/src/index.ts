...
import { createAlphacode, type ToolPart } from "@alphacode-ai/sdk"
...
console.log("🚀 Starting alphacode server...")
const alphacode = await createAlphacode({
  port: 0,
})
...
void (async () => {
  const events = await alphacode.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new alphacode session...")
    const { client, server } = alphacode
    ...
    console.log("✅ Created alphacode session:", createResult.data.id)
  ...
  console.log("📝 Sending to alphacode:", message.text)
