...
Le skill degli agenti permettono a AlphaCode di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
AlphaCode cerca in queste posizioni:

- Config di progetto: `.alphacode/skills/<name>/SKILL.md`
- Config globale: `~/.config/alphacode/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, AlphaCode risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.alphacode/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/alphacode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.alphacode/skills/git-release/SKILL.md` cosi':
...
AlphaCode elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `alphacode.json`:
...
**Per agenti integrati** (in `alphacode.json`):
