...
import { and, Database, eq, isNull, lt, or, sql } from "@alphacode-ai/console-core/drizzle/index.js"
import { KeyTable } from "@alphacode-ai/console-core/schema/key.sql.js"
import { BillingTable, LiteTable, SubscriptionTable, UsageTable } from "@alphacode-ai/console-core/schema/billing.sql.js"
import { centsToMicroCents } from "@alphacode-ai/console-core/util/price.js"
import { getMonthlyBounds, getWeekBounds } from "@alphacode-ai/console-core/util/date.js"
import { Identifier } from "@alphacode-ai/console-core/identifier.js"
import { Billing } from "@alphacode-ai/console-core/billing.js"
import { Actor } from "@alphacode-ai/console-core/actor.js"
import { WorkspaceTable } from "@alphacode-ai/console-core/schema/workspace.sql.js"
import { ZenData } from "@alphacode-ai/console-core/model.js"
import { Subscription } from "@alphacode-ai/console-core/subscription.js"
import { BlackData } from "@alphacode-ai/console-core/black.js"
import { UserTable } from "@alphacode-ai/console-core/schema/user.sql.js"
import { ModelTable } from "@alphacode-ai/console-core/schema/model.sql.js"
import { ProviderTable } from "@alphacode-ai/console-core/schema/provider.sql.js"
...
import { LiteData } from "@alphacode-ai/console-core/lite.js"
import { Resource } from "@alphacode-ai/console-resource"
...
import { Workspace } from "@alphacode-ai/console-core/workspace.js"
...
export async function handler(
  ...
  try {
    ...
    const sessionId = input.request.headers.get("x-alphacode-session") ?? ""
    const requestId = input.request.headers.get("x-alphacode-request") ?? ""
    const ocClient = input.request.headers.get("x-alphacode-client") ?? ""
    const projectId = input.request.headers.get("x-alphacode-project") ?? ""
    ...
    if (
      ...
      throw new RegionError(
        t("zen.api.error.regionNotAllowed", {
          consoleGoUrl: `https://alphacode.ai/workspace/${authInfo.workspaceID}/go`,
        }),
      )
    ...
    const retriableRequest = async (retry: RetryOptions = { excludeProviders: [], retryCount: 0 }) => {
      ...
      const res = await fetchWithRetryableStatus(
        reqUrl,
        {
          ...
          headers: (() => {
            ...
            headers.delete("x-alphacode-request")
            headers.delete("x-alphacode-session")
            headers.delete("x-alphacode-project")
            headers.delete("x-alphacode-client")
            return headers
          })(),
          body: reqBody,
      ...
      if (isNewInference) {
        const resEndpointId = res.headers.get("x-alphacode-endpoint-id")
        const resEndpointModelId = res.headers.get("x-alphacode-upstream-model-id")
  ...
  function validateBilling(authInfo: AuthInfo, modelInfo: ModelInfo): BillingSource {
    ...
    if (opts.modelList === "lite" && authInfo.billing.lite && authInfo.lite) {
      try {
        const consoleGoUrl = `https://alphacode.ai/workspace/${authInfo.workspaceID}/go`
    ...
    const billingUrl = `https://alphacode.ai/workspace/${authInfo.workspaceID}/billing`
    const membersUrl = `https://alphacode.ai/workspace/${authInfo.workspaceID}/members`
