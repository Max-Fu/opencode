...
The alphacode server has moved to the Effect HttpApi backend. Remaining work is
...
- remove the public in-memory `@alphacode-ai/llm` tool loop after replacing its
...
- plugins get global "alphacode" instance like in that post i showed
- alphacode instance has stuff like `alphacode.session.prompt()` or
  `alphacode.tool.register({...})`
...
- decide whether to preserve deprecated `@alphacode-ai/llm` orchestration exports
