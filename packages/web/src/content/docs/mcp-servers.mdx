...
You can add external tools to AlphaCode using the _Model Context Protocol_, or MCP. AlphaCode supports both local and remote servers.
...
You can define MCP servers in your [AlphaCode Config](https://alphacode.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/alphacode` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
AlphaCode automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, AlphaCode will:
...
If the server requires authentication, AlphaCode will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `alphacode mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, AlphaCode will store the tokens securely in `~/.local/share/alphacode/mcp-auth.json`.
...
Your MCPs are available as tools in AlphaCode, alongside built-in tools. So you can manage them through the AlphaCode config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect AlphaCode to your Sentry account.
