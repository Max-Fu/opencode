...
import type { IntegrationOAuthMethodRegistration } from "@alphacode-ai/plugin/v2/effect/integration"
import { define } from "@alphacode-ai/plugin/v2/effect/plugin"
...
const defaultServer = "https://console.alphacode.ai"
const clientID = "alphacode-cli"
...
export const AlphacodePlugin = define<HttpClient.HttpClient | EventV2.Service | Scope.Scope>({
  id: "alphacode",
  effect: Effect.fn(function* (ctx) {
    ...
    const load = Effect.fn("AlphacodePlugin.load")(function* () {
      const connection = yield* ctx.integration.connection.active("alphacode")
    ...
    yield* ctx.integration.transform((draft) => {
      draft.update("alphacode", (integration) => {
        integration.name = "AlphaCode"
      })
      draft.method.update(oauth(http))
      draft.method.update({ integrationID: "alphacode", method: { type: "key", label: "API key (service account)" } })
    })

    connected = (yield* ctx.integration.connection.active("alphacode")) !== undefined
    yield* ctx.catalog.transform((catalog) => {
      for (const [providerID, item] of Object.entries(providers ?? {})) {
        catalog.provider.update(providerID, (provider) => {
          provider.integrationID = Integration.ID.make("alphacode")
      ...
      const item = catalog.provider.get(ProviderV2.ID.alphacode)
      if (!item) return
      const hasKey = Boolean(process.env.ALPHACODE_API_KEY || connected || item.provider.request.body.apiKey)
    ...
    yield* events.subscribe(Integration.Event.ConnectionUpdated).pipe(
      Stream.filter((event) => event.data.integrationID === Integration.ID.make("alphacode")),
