...
Puede agregar herramientas externas a AlphaCode utilizando el _Model Context Protocol_ o MCP. AlphaCode admite servidores locales y remotos.
...
Puede definir servidores MCP en su [AlphaCode Config](https://alphacode.ai/docs/config/) en `mcp`. Agregue cada MCP con un nombre único. Puede referirse a ese MCP por su nombre cuando solicite el LLM.
...
Las organizaciones pueden proporcionar servidores MCP predeterminados a través de su punto final `.well-known/alphacode`. Estos servidores pueden estar deshabilitados de forma predeterminada, lo que permite a los usuarios optar por los que necesitan.
...
AlphaCode maneja automáticamente la autenticación OAuth para servidores MCP remotos. Cuando un servidor requiere autenticación, AlphaCode:
...
Si el servidor requiere autenticación, AlphaCode le pedirá que se autentique cuando intente usarlo por primera vez. De lo contrario, puede [activar manualmente el flujo](#authenticating) con `alphacode mcp auth <server-name>`.
...
El comando `mcp auth` abrirá su navegador para obtener autorización. Después de su autorización, AlphaCode almacenará los tokens de forma segura en `~/.local/share/alphacode/mcp-auth.json`.
...
Sus MCP están disponibles como herramientas en AlphaCode, junto con herramientas integradas. Para que puedas administrarlos a través de la configuración AlphaCode como cualquier otra herramienta.
...
Esto abrirá una ventana del navegador para completar el flujo OAuth y conectar AlphaCode a su cuenta Sentry.
