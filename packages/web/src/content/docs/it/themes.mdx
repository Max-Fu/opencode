...
Con AlphaCode puoi scegliere tra diversi temi integrati, usare un tema che si adatta al tema del tuo terminale oppure definire un tema personalizzato.

Per impostazione predefinita, AlphaCode usa il tema `alphacode`.
...
AlphaCode include diversi temi integrati.
...
- Vuole che AlphaCode corrisponda all'aspetto del terminale
...
AlphaCode supporta un sistema di temi flessibile basato su JSON che permette di creare e personalizzare temi facilmente.
...
2. **Directory di configurazione utente** - in `~/.config/alphacode/themes/*.json` o `$XDG_CONFIG_HOME/alphacode/themes/*.json`
3. **Directory root del progetto** - in `<project-root>/.alphacode/themes/*.json`
4. **Directory di lavoro corrente** - in `./.alphacode/themes/*.json`
