...
Her er alle de innebygde verktøyene som er tilgængelige i AlphaCode.
...
:::note
Dette verktøyet er bare tilgængelig når `ALPHACODE_EXPERIMENTAL_LSP_TOOL=true` (eller `ALPHACODE_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger AlphaCode-leverandøren eller når miljøvariabelen `ALPHACODE_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
