...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@alphacode/acme")).toBe("@alphacode/acme")
    expect(Npm.sanitize("@alphacode/acme@1.0.0")).toBe("@alphacode/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/alphacode/acme.git"
    const expected = win ? "acme@git+https_//github.com/alphacode/acme.git" : spec
