...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "alphacode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "alphacode.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "alphacode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "alphacode.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["alphacode.json", "alphacode.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "alphacode.json"),
        path.join(project, "alphacode.jsonc"),
      ])
    })
  })
