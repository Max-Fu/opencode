...
Aby debugować problemy z alphacode, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\alphacode\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `alphacode --log-level DEBUG`.

---

## Przechowywanie danych

alphacode przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/alphacode/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\alphacode`
...
alphacode Desktop uruchamia w tle lokalny serwer alphacode (sidecar `alphacode-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `AlphaCode` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/alphacode/alphacode.jsonc` (lub `~/.config/alphacode/alphacode.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/alphacode/alphacode.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\alphacode\alphacode.jsonc`
...
alphacode może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/alphacode/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\alphacode\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby alphacode mógł ją odbudować.

1. Całkowicie zamknij alphacode Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/alphacode`
- **Linux**: usuń `~/.cache/alphacode` (lub uruchom `rm -rf ~/.cache/alphacode`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\alphacode`

3. Uruchom ponownie alphacode Desktop.

---

### Napraw problemy z połączeniem z serwerem

alphacode Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `alphacode.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `ALPHACODE_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `ALPHACODE_PORT` (lub wybierz wolny port) i uruchom ponownie.
...
W systemie Windows alphacode Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami alphacode.

---

### Brak powiadomień

alphacode Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla alphacode w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie alphacode Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji alphacode Desktop):

- `alphacode.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `alphacode.global.dat` i `alphacode.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
Jeśli masz problemy z alphacode:
...
3. Upewnij się, że masz najnowszą wersję z `alphacode upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `alphacode models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\alphacode`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. alphacode dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\alphacode`

2. Uruchom ponownie kod alphacode, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to alphacode do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
alphacode wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
