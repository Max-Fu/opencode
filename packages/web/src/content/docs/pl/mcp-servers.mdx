...
Możesz dodać dodatkowe informacje do alphacode za pomocą _Model Context Protocol_, w skrócie MCP. alphacode obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [alphacode Config](https://alphacode.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/alphacode`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
alphacode automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, alphacode:
...
Jeśli serwer wymaga uwierzytelnienia, alphacode poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `alphacode mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji alphacode bezpieczne przechowa tokeny w `~/.local/share/alphacode/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w alphacode, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie alphacode, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć alphacode z kontem Sentry.
