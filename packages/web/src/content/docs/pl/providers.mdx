---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w alphacode.
---

import config from "../../../../config.mjs"
export const console = config.console

alphacode używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji alphacode.
...
w `~/.local/share/alphacode/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim alphacode
config.
...
AlphaCode Zen to lista modeli dostarczonych przez zespół alphacode, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z alphacode. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od AlphaCode Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `AlphaCode Zen` i przejdź do [alphacode.ai/auth](https://alphacode.ai/zen).
...
Działa jak każdy inny dostawca w alphacode i jest całkowicie opcjonalny w użyciu.

---

## AlphaCode Go

AlphaCode Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół alphacode, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z alphacode.

1. Uruchom polecenie `/connect` w TUI, wybierz `AlphaCode Go` i przejdź do [alphacode.ai/auth](https://alphacode.ai/zen).
...
Aby używać Amazon Bedrock z alphacode:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania alphacode:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `alphacode.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w alphacode nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować alphacode do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod alphacode działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji alphacode.
...
alphacode używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować alphacode
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`alphacode.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [alphacode-gitlab-auth](https://www.npmjs.com/package/alphacode-gitlab-auth).
...
Aby korzystać z subskrypcji GitHub Copilot z kodem alphacode:
...
Aby używać Google Vertex AI z alphacode:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania alphacode.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą alphacode, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [alphacode-helicone-session](https://github.com/H2Shami/alphacode-helicone-session), aby automatycznie rejestrować każdą konwersację alphacode jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja alphacode będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować alphacode tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować alphacode tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować alphacode tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem alphacode. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/alphacode).
:::
...
Aby korzystać z Ollama Cloud z alphacode:
...
3. Skopiuj klucz API do użycia w alphacode.
...
6. **Ważne**: Przed użyciem modeli chmurowych w alphacode musisz pobrać informacje o modelu lokalnie:
...
AlphaCode Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół alphacode. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>AlphaCode Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **AlphaCode Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API alphacode.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji alphacode.
...
5. Możesz je także dostosować za pomocą konfiguracji alphacode. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z alphacode:
...
Możesz także dostosować modele za pomocą konfiguracji alphacode. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z alphacode. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `alphacode.json` w katalogu projektu:
...
Pola `limit` pozwalają alphacode zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `alphacode auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację alphacode i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji alphacode.
