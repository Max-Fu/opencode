...
import { define } from "@alphacode-ai/plugin/v2/effect"
...
import { LocationServiceMap } from "@alphacode-ai/core/location-services"
      ...
      Effect.flatMap(([blocked, allowed]) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(blocked.path, "alphacode.json"),
              JSON.stringify({
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((dir) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(dir.path, "alphacode.json"),
              JSON.stringify({
                providers: {
                  unavailable: {
                    name: "Unavailable",
