...
let releaseAlphacodeResolve: (() => void) | undefined
...
test("rejects an update that did not install the desktop version", () => {
  expect(() => expectAlphacodeVersion("1.16.2", "1.16.2")).not.toThrow()
  expect(() => expectAlphacodeVersion("1.14.35", "1.16.2")).toThrow(
...
test("clears cached distro probes when removing a WSL server", () => {
  expect(
    clearWslDistroState(
      { Debian: { name: "Debian", canExecute: true, hasBash: true, hasCurl: true, error: null } },
      {
        Debian: {
          distro: "Debian",
          resolvedPath: "/home/luke/.alphacode/bin/alphacode",
          version: "1.16.2",
  ...
  ).toEqual({ distroProbes: {}, alphacodeChecks: {} })
})
...
test("ignores stale background AlphaCode checks after removing a WSL server", async () => {
  persistedServers = []
  releaseAlphacodeResolve = undefined
  ...
  await waitFor(() => !!releaseAlphacodeResolve)
  await controller.removeServer("wsl:Debian")
  releaseAlphacodeResolve?.()
  await new Promise((resolve) => setTimeout(resolve, 0))

  expect(controller.getState().servers).toEqual([])
  expect(controller.getState().alphacodeChecks).toEqual({})
})
...
test("probes addable distros in parallel before checking AlphaCode", async () => {
  ...
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...
    resolveAlphacode: async (distro) => {
      alphacode.push(distro)
      return "/home/me/.alphacode/bin/alphacode"
    },
  })
  ...
  expect(alphacode).toEqual([])
  release.get("Debian")?.()
  release.get("Ubuntu")?.()
  await task

  expect(Object.keys(controller.getState().distroProbes)).toEqual(["Debian", "Ubuntu"])
  expect(alphacode).toEqual(["Debian", "Ubuntu"])
  expect(Object.keys(controller.getState().alphacodeChecks)).toEqual(["Debian", "Ubuntu"])
})
...
test("does not check AlphaCode in addable distros that cannot execute commands", async () => {
  ...
  expect(Object.keys(controller.getState().distroProbes)).toEqual(["Debian", "Ubuntu"])
  expect(alphacode).toEqual(["Debian"])
  expect(Object.keys(controller.getState().alphacodeChecks)).toEqual(["Debian"])
})
...
function testControllerOptions() {
  return {
    ...
    resolveAlphacode: async () => {
      await new Promise<void>((resolve) => {
        releaseAlphacodeResolve = resolve
      })
      return "/home/me/.alphacode/bin/alphacode"
    },
  }
