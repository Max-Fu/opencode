...
describe("alphacode acp initialize/auth subprocess", () => {
  cliIt.live(
    "initialize responds with capabilities",
    ({ alphacode }) =>
      Effect.gen(function* () {
        const initialized = yield* initialize(yield* createAcpClient({ alphacode }))
  ...
  cliIt.live(
    ...
    ({ alphacode }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ alphacode })
        const initialized = yield* initialize(acp)

        expect(initialized.authMethods?.[0]?.id).toBe("alphacode-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "alphacode-login" })).toMatchObject({
          result: {},
        })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.ALPHACODE_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ alphacode }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("alphacode-login")
