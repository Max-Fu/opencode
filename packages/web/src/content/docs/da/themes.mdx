...
Med AlphaCode kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger AlphaCode vårt eget `alphacode`-tema.
...
AlphaCode støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/alphacode/themes/*.json` eller `$XDG_CONFIG_HOME/alphacode/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.alphacode/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.alphacode/themes/*.json`
