...
export type AddServerPrimaryButton = {
  ...
  label: AddServerText
  disabled: boolean
  action: "install-alphacode" | "add" | null
  loading: boolean
  width: string | null
}
...
export function wslAlphacodeAction(check?: WslAlphacodeCheck) {
  if (!check) return
...
export function addServerViewModel(input: {
  ...
  const alphacodeCheck = selectedDistro ? (state?.alphacodeChecks[selectedDistro] ?? null) : null
...
function addServerDistroStatus(input: {
  ...
  const check = input.state?.alphacodeChecks[input.name]
  if (!check) {
    if (input.probingAddable || (job?.kind === "probe-addable" && job.distros.includes(input.name))) {
      return checkingStatus()
    }
  ...
  if (!check.resolvedPath) return { label: { key: "wsl.onboarding.distroStatus.alphacodeMissing" }, tone: "warning" }
...
function addServerPrimaryButton(input: {
  state: WslServersState | undefined
  selectedDistro: string | null
  alphacodeCheck: WslAlphacodeCheck | null
  adding: boolean
  probingAddable: boolean
  ...
  const probingAlphacode =
    probingSelected ||
    (ready &&
      (!input.alphacodeCheck ||
        (!!input.selectedDistro &&
          input.state?.job?.kind === "probe-addable" &&
          input.state.job.distros.includes(input.selectedDistro))))
  const installingAlphacode =
    input.state?.job?.kind === "install-alphacode" && input.state.job.distro === input.selectedDistro
  if (!ready || probingAlphacode) {
    return {
      variant: "contrast",
  ...
  if (!addServerAlphacodeReady(input.alphacodeCheck)) {
    const update = !!input.alphacodeCheck?.resolvedPath && input.alphacodeCheck.matchesDesktop === false
    return {
      variant: "neutral",
      label: installingAlphacode
      ...
      action: "install-alphacode",
      loading: installingAlphacode,
...
function addServerAlphacodeReady(check: WslAlphacodeCheck | null) {
...
function addServerSelectedDistroSettled(state: WslServersState | undefined, selectedDistro: string | null) {
  ...
  return !!state.alphacodeChecks[selectedDistro]
}
...
export function addableProbePlan(input: {
  ...
  const pending = ordered.flatMap((item) => {
    ...
    if (wslDistroReady(state, item.name) && !state.alphacodeChecks[item.name]) return [`alphacode:${item.name}`]
    return []
  })
