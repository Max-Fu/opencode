...
import { type Session } from "@alphacode-ai/sdk/v2/client"
import {
  childSessionOnPath,
  closeHomeProject,
  compareSessionTime,
...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("alphacode://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("alphacode://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("alphacode://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("alphacode://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("alphacode://open-project")).toBeUndefined()
    expect(parseDeepLink("alphacode://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "alphacode://open-project?directory=/a",
      "alphacode://other?directory=/b",
      "alphacode://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("alphacode://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("alphacode://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("alphacode://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("alphacode://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "alphacode://new-session?directory=/a",
      "alphacode://open-project?directory=/b",
      "alphacode://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __ALPHACODE__: {
        deepLinks: ["alphacode://open-project?directory=/a"],
      },
    } as unknown as Window & { __ALPHACODE__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["alphacode://open-project?directory=/a"])
