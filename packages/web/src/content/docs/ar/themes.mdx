...
مع AlphaCode يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم AlphaCode سمتنا `alphacode`.
...
يأتي AlphaCode مع عدة سمات مدمجة.
...
- يريدون أن يطابق AlphaCode مظهر terminal لديهم
...
يدعم AlphaCode نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/alphacode/themes/*.json` أو `$XDG_CONFIG_HOME/alphacode/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.alphacode/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.alphacode/themes/*.json`
