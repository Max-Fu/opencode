...
Both `action` and `resource` use alphacode's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.alphacode` files is intentionally deferred until `.alphacode` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether alphacode permits an operation through its managed execution paths.
