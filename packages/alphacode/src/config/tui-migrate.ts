...
const TUI_SCHEMA_URL = "https://alphacode.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from alphacode.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const alphacode = await alphacodeFiles(input)
  for (const file of alphacode) {
...
async function alphacodeFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "alphacode"),
    ...(await Filesystem.findUp(["alphacode.json", "alphacode.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "alphacode"))
  }
  if (Flag.ALPHACODE_CONFIG) files.push(Flag.ALPHACODE_CONFIG)
