---
title: サーバー
description: HTTP 経由で AlphaCode サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`alphacode serve` コマンドは、AlphaCode クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
| `--mdns-domain` | mDNS サービスのカスタムドメイン名 | `alphacode.local` |
...
HTTP 基本認証でサーバーを保護するには、`ALPHACODE_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `alphacode` になるか、`ALPHACODE_SERVER_USERNAME` を設定してオーバーライドします。これは、`alphacode serve` と `alphacode web` の両方に当てはまります。
...
`alphacode` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
AlphaCode サーバーを使用して、プログラムで AlphaCode と対話します。
:::

`alphacode serve` を実行してスタンドアロンサーバーを起動できます。すでに AlphaCode TUI を実行している場合でも、`alphacode serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、AlphaCode [IDE](/docs/ide) プラグインによって使用されます。
