---
title: Plugins
description: Skriv dine egne plugins for at udvide AlphaCode.
---

Plugins giver dig mulighed for at udvide AlphaCode ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre AlphaCodes standardadfærd.
...
- `.alphacode/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/alphacode/node_modules/`.
...
1. Global konfiguration (`~/.config/alphacode/alphacode.json`)
2. Projektkonfiguration (`alphacode.json`)
3. Global plugin-mappe (`~/.config/alphacode/plugins/`)
4. Projekt plugin bibliotek (`.alphacode/plugins/`)
...
AlphaCode kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En alphacode SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide alphacode.
...
:::note
Hvis du bruger AlphaCode desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå alphacode i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til alphacode:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som alphacode kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for alphacode sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://alphacode.ai/docs/sdk) for detaljer.
