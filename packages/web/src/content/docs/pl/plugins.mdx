---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć alphacode.
---

Wtyczki umożliwiające wyłączenie alphacode poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania alphacode.
...
- `.alphacode/plugins/` - Wtyczki na poziomie projektu
- `~/.config/alphacode/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/alphacode/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/alphacode/alphacode.json`)
2. Konfiguracja projektu (`alphacode.json`)
3. Globalny katalog wtyczek (`~/.config/alphacode/plugins/`)
4. Katalog wtyczek projektu (`.alphacode/plugins/`)
...
alphacode uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z alphacode do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do alphacode.
...
:::note
Jeśli korzystasz z aplikacji komputerowej alphacode, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj alphacode czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do alphacode:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod alphacode. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://alphacode.ai/docs/sdk).
