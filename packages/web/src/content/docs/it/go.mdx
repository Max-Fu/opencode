...
AlphaCode Go è un abbonamento a basso costo — **5 $ per il primo mese**, poi **10 $/mese** — che ti offre un accesso affidabile ai popolari modelli di programmazione aperti.

Go funziona come qualsiasi altro provider in AlphaCode. Ti abboni a AlphaCode Go e
ottieni la tua chiave API. È **completamente facoltativo** e non hai bisogno di usarlo per
utilizzare AlphaCode.
...
AlphaCode Go ti dà accesso a questi modelli a **5 $ per il primo mese**, poi a **10 $/mese**.

---

## Come funziona

AlphaCode Go funziona come qualsiasi altro provider in AlphaCode.

1. Accedi a **<a href={console}>AlphaCode Zen</a>**, ti abboni a Go e
   copi la tua chiave API.
2. Esegui il comando `/connect` nella TUI, selezioni `AlphaCode Go` e incolli
   la tua chiave API.
3. Esegui `/models` nella TUI per vedere l'elenco dei modelli disponibili tramite Go.

:::note
Solo un membro per workspace può abbonarsi a AlphaCode Go.
:::
...
AlphaCode Go include i seguenti limiti:
...
Il [model id](/docs/config/#models) nella tua AlphaCode config
utilizza il formato `alphacode-go/<model-id>`. Ad esempio, per Kimi K3, useresti
`alphacode-go/kimi-k3` nella tua configurazione.
...
Abbiamo creato AlphaCode Go per:
...
4. Non avere **nessun lock-in**, permettendoti di utilizzare anche qualsiasi altro provider con AlphaCode.
