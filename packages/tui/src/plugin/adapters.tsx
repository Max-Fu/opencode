import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@alphacode-ai/plugin/tui"
...
import type { useAlphacodeKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useAlphacodeKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getAlphacodeModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getAlphacodeModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
