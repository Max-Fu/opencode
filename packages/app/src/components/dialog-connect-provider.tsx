...
import { Dialog } from "@alphacode-ai/ui/dialog"
import { Icon } from "@alphacode-ai/ui/icon"
import { IconButton } from "@alphacode-ai/ui/icon-button"
import { List, type ListRef } from "@alphacode-ai/ui/list"
import { ProviderIcon } from "@alphacode-ai/ui/provider-icon"
import { Spinner } from "@alphacode-ai/ui/spinner"
import { Tag } from "@alphacode-ai/ui/tag"
import { TextField } from "@alphacode-ai/ui/text-field"
import { ButtonV2 } from "@alphacode-ai/ui/v2/button-v2"
import { DialogBody, DialogHeader, DialogTitle, DialogV2 } from "@alphacode-ai/ui/v2/dialog-v2"
import { TextInputV2 } from "@alphacode-ai/ui/v2/text-input-v2"
...
function ProviderPicker(props: {
  ...
  const note = (id: string) => {
    ...
    if (id === "alphacode-go") return language.t("dialog.provider.alphacodeGo.tagline")
    return undefined
  }
    ...
    >
      {(i) => (
        <div class="px-1.25 w-full flex items-center gap-x-3">
          ...
          <span>{i.name}</span>
          <Show when={i.id === "alphacode"}>
            <div class="text-14-regular text-text-weak">{language.t("dialog.provider.alphacode.tagline")}</div>
          </Show>
...
function ProviderPickerV2(props: {
  ...
  const featured = ["alphacode", "alphacode-go", "anthropic", "openai", "google", "openrouter", "vercel"]
                  ...
                  <For each={group.items()}>
                    {(provider) => (
                      ...
                      >
                        ...
                        <Show when={provider.id === "alphacode" || provider.id === "alphacode-go"}>
                          <span class="min-w-0 truncate font-[440] text-v2-text-text-muted">
                            {language.t(
                              provider.id === "alphacode"
                                ? "dialog.provider.alphacode.tagline"
                                : "dialog.provider.alphacodeGo.tagline",
                            )}
                          </span>
  ...
  function ApiAuthView() {
    ...
    if (newLayout())
      return (
        <div class="flex flex-col gap-5 px-3 text-[13px] font-[440] leading-5 tracking-[-0.04px] text-v2-text-text-muted">
          <Show
            when={provider().id === "alphacode"}
            fallback={language.t("provider.connect.apiKey.description", { provider: provider().name })}
          >
            <div class="flex flex-col gap-5">
              <div>{language.t("provider.connect.alphacodeZen.line1")}</div>
              <div>{language.t("provider.connect.alphacodeZen.line2")}</div>
              <div>
                {language.t("provider.connect.alphacodeZen.visit.prefix")}
                <ExternalLink
                  href="https://alphacode.ai/zen"
                  class="text-v2-text-text-base focus-visible:rounded-xs focus-visible:outline-2 focus-visible:outline-v2-border-border-focus"
                >
                  {language.t("provider.connect.alphacodeZen.visit.link")}
                </ExternalLink>
                {language.t("provider.connect.alphacodeZen.visit.suffix")}
              </div>
            </div>
          </Show>
    ...
    return (
      <div class="flex flex-col gap-6">
        <Switch>
          <Match when={provider().id === "alphacode"}>
            <div class="flex flex-col gap-4">
              <div class="text-14-regular text-text-base">{language.t("provider.connect.alphacodeZen.line1")}</div>
              <div class="text-14-regular text-text-base">{language.t("provider.connect.alphacodeZen.line2")}</div>
              <div class="text-14-regular text-text-base">
                {language.t("provider.connect.alphacodeZen.visit.prefix")}
                <ExternalLink href="https://alphacode.ai/zen" tabIndex={-1}>
                  {language.t("provider.connect.alphacodeZen.visit.link")}
                </ExternalLink>
                {language.t("provider.connect.alphacodeZen.visit.suffix")}
              </div>
