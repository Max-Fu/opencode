...
Avec AlphaCode, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, AlphaCode utilise notre propre thème `alphacode`.
...
AlphaCode est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que AlphaCode corresponde à l'apparence de leur terminal
...
AlphaCode prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/alphacode/themes/*.json` ou `$XDG_CONFIG_HOME/alphacode/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.alphacode/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.alphacode/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/alphacode/themes
vim ~/.config/alphacode/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .alphacode/themes
vim .alphacode/themes/my-theme.json
```
...
{
  "$schema": "https://alphacode.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
