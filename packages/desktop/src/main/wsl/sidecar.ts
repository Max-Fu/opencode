...
import { type WslCommandLine, resolveWslAlphacode, shellEscape, wslArgs } from "./runtime"
...
export async function spawnWslSidecar(
  ...
  const alphacode = await resolveWslAlphacode(distro)
  if (!alphacode) throw new Error(nativeT("desktop.wsl.error.alphacodeNotInstalled", { distro }))

  const port = await allocatePort()
  const password = randomUUID()
  const username = "alphacode"
  const script = [
    "set -euo pipefail",
    'cd "$HOME" || cd /',
    ...
    "export PATH",
    "export WSLENV=",
    "export ALPHACODE_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export ALPHACODE_CLIENT=desktop",
    `export ALPHACODE_SERVER_USERNAME=${shellEscape(username)}`,
    `export ALPHACODE_SERVER_PASSWORD=${shellEscape(password)}`,
    'export XDG_STATE_HOME="$HOME/.local/state"',
    `exec ${shellEscape(alphacode)} --print-logs --log-level ${app.isPackaged ? "WARN" : "INFO"} serve --hostname 0.0.0.0 --port ${port}`,
  ].join("\n")
