...
import { registerAlphacodeSpinner } from "./component/register-spinner"
...
import {
  ...
  ALPHACODE_BASE_MODE,
  AlphacodeKeymapProvider,
  registerAlphacodeKeymap,
  useBindings,
  useAlphacodeKeymap,
} from "./keymap"
  ...
  const result = yield* Effect.scoped(
    Effect.gen(function* () {
        ...
        Effect.tryPromise({
          try: () =>
            createCliRenderer({
              ...
              useMouse: !Flag.ALPHACODE_DISABLE_MOUSE && input.config.mouse,
              consoleOptions: {
      ...
      yield* Effect.acquireRelease(
        Effect.sync(() => registerAlphacodeKeymap(keymap, renderer, input.config)),
                ...
                <ErrorBoundary fallback={(error, reset) => <ErrorComponent error={error} reset={reset} mode={mode} />}>
                  ...
                  >
                    ...
                    >
                      <TuiStartupProvider
                        value={{
                          initialRoute: process.env.ALPHACODE_ROUTE ? JSON.parse(process.env.ALPHACODE_ROUTE) : undefined,
                          skipInitialLoading: Boolean(process.env.ALPHACODE_FAST_BOOT),
                        }}
                      >
                        <ClipboardProvider>
                          <AlphacodeKeymapProvider keymap={keymap}>
...
function App(props: { onSnapshot?: () => Promise<string[]>; pluginHost: TuiPluginHost }) {
  ...
  const offSelectionKeys = keymap.intercept(
    "key",
    ({ event }) => {
      if (!Flag.ALPHACODE_EXPERIMENTAL_DISABLE_COPY_ON_SELECT) return
  ...
  createEffect(() => {
    if (!terminalTitleEnabled() || Flag.ALPHACODE_DISABLE_TERMINAL_TITLE) return
  ...
  const appCommands = createMemo(() =>
    [
      ...
      },
      {
        ...
        category: "Workspace",
        hidden: !Flag.ALPHACODE_EXPERIMENTAL_WORKSPACES,
      ...
      },
      {
        ...
        title: "Open docs",
        run: () => {
          open("https://alphacode.ai/docs").catch(() => {})
          dialog.clear()
        },
        category: "System",
      },
  ...
  return (
    <box
      ...
      onMouseUp={
        !Flag.ALPHACODE_EXPERIMENTAL_DISABLE_COPY_ON_SELECT
          ? () => Selection.copy(renderer, toast, clipboard)
          : undefined
      }
    >
      <Show when={Flag.ALPHACODE_SHOW_TTFD}>
        <TimeToFirstDraw />
      </Show>
      <Show when={ready()}>
