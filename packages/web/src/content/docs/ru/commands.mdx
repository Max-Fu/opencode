...
Создайте `.alphacode/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию alphacode или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем alphacode [config](/docs/config):
...
- Глобальный: `~/.config/alphacode/commands/`
- Для каждого проекта: `.alphacode/commands/`
...
alphacode включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
