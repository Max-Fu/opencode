...
Agent skills omogucavaju AlphaCode da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
AlphaCode pretrazuje ove lokacije:

- Konfiguracija projekta: `.alphacode/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/alphacode/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, AlphaCode ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.alphacode/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/alphacode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.alphacode/skills/git-release/SKILL.md` ovako:
...
AlphaCode navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `alphacode.json`:
...
**Za ugradene agente** (u `alphacode.json`):
