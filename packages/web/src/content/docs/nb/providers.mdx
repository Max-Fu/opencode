---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i AlphaCode.
---

import config from "../../../../config.mjs"
export const console = config.console

AlphaCode bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i AlphaCode-konfigurasjonen.
...
i `~/.local/share/alphacode/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i AlphaCode
konfig.
...
AlphaCode Zen er en liste over modeller levert av AlphaCode-teamet som har vært
testet og verifisert for å fungere godt med AlphaCode. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med AlphaCode Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `AlphaCode Zen` og gå til [alphacode.ai/auth](https://alphacode.ai/zen).
...
Det fungerer som alle andre leverandører i AlphaCode og er helt valgfritt å bruke.

---

## AlphaCode Go

AlphaCode Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av AlphaCode-teamet som har vært
testet og verifisert for å fungere godt med AlphaCode.

1. Kjør kommandoen `/connect` i TUI, velg `AlphaCode Go`, og gå til [alphacode.ai/auth](https://alphacode.ai/zen).
...
Slik bruker du Amazon Bedrock med AlphaCode:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører AlphaCode:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `alphacode.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i alphacode støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere alphacode til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at AlphaCode skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom AlphaCode-konfigurasjonen.
...
for å lagre token i AlphaCode auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
AlphaCode bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse AlphaCode
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`alphacode.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [alphacode-gitlab-auth](https://www.npmjs.com/package/alphacode-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `alphacode.json`:
...
Slik bruker du GitHub Copilot-abonnementet med AlphaCode:
...
Slik bruker du Google Vertex AI med AlphaCode:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører AlphaCode.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom AlphaCode, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [alphacode-helicone-session](https://github.com/H2Shami/alphacode-helicone-session) plugin for automatisk å logge hver AlphaCode-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver AlphaCode-samtale oppført som en separat økt.
...
Du kan konfigurere AlphaCode for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere AlphaCode for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere AlphaCode for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for AlphaCode. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/alphacode) for detaljer.
:::
...
Slik bruker du Ollama Cloud med AlphaCode:
...
3. Kopier API-nøkkelen for bruk i AlphaCode.
...
6. **Viktig**: Før du bruker skymodeller i AlphaCode, må du hente modellinformasjonen lokalt:
...
AlphaCode Zen er en liste over testede og verifiserte modeller levert av AlphaCode-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>AlphaCode Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **AlphaCode Zen**.

   ```txt
   /connect
   ```

3. Skriv inn AlphaCode API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom AlphaCode-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom AlphaCode-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med AlphaCode:
...
Du kan også tilpasse modeller gjennom AlphaCode-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med AlphaCode. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `alphacode.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar AlphaCode forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `alphacode auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk AlphaCode-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i AlphaCode-konfigurasjonen.
