---
title: Regeln
description: Lege eigene Anweisungen fuer alphacode fest.
---

Du kannst alphacode ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in alphacode den Befehl `/init` aus.
...
So kann alphacode sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht alphacode sie zu erweitern.
...
alphacode kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/alphacode/AGENTS.md` speichern.
Diese gelten in allen alphacode-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt AlphaCode dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/alphacode/AGENTS.md` existiert)
...
Beim Start sucht alphacode in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/alphacode/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `alphacode.json` oder in `~/.config/alphacode/alphacode.json` angeben.
...
alphacode parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit alphacode.json

Empfohlen ist das `instructions`-Feld in der `alphacode.json`:
...
Du kannst alphacode explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `alphacode.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
