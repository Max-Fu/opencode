---
title: Server
description: Interact with alphacode server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `alphacode serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an alphacode client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `alphacode.local` |
...
Set `ALPHACODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `alphacode`, or set `ALPHACODE_SERVER_USERNAME` to override it. This applies to both `alphacode serve` and `alphacode web`.
...
When you run `alphacode` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the alphacode server to interact with alphacode programmatically.
:::

This architecture lets alphacode support multiple clients and allows you to interact with alphacode programmatically.

You can run `alphacode serve` to start a standalone server. If you have the
alphacode TUI running, `alphacode serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the AlphaCode [IDE](/docs/ide) plugins.
...
The alphacode server exposes the following APIs.
