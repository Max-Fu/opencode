---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i AlphaCode.
---

import config from "../../../../config.mjs"
export const console = config.console

AlphaCode bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i AlphaCode-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/alphacode/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i AlphaCode-konfigurationen.
...
AlphaCode Zen er en liste over modeller leveret af AlphaCode-teamet, som er blevet testet og verificeret til at fungere godt med AlphaCode. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med AlphaCode Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg alphacode og gå til [alphacode.ai/auth](https://alphacode.ai/auth).
...
Det fungerer som alle andre udbydere i AlphaCode og er helt valgfrit at bruge.

---

## AlphaCode Go

AlphaCode Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af AlphaCode-teamet, som er testet og verificeret til at fungere godt med AlphaCode.

1. Kør kommandoen `/connect` i TUI, vælg `AlphaCode Go`, og gå til [alphacode.ai/auth](https://alphacode.ai/zen).
...
Sådan bruger du Amazon Bedrock med AlphaCode:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører alphacode:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `alphacode.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i AlphaCode understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere alphacode til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at alphacode skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem alphacode-konfigurationen.
...
gemme tokenet i alphacode auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
AlphaCode bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse AlphaCode
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`alphacode.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [alphacode-gitlab-auth](https://www.npmjs.com/package/alphacode-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `alphacode.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med AlphaCode:
...
Sådan bruger du Google Vertex AI med AlphaCode:
...
2. Angiv de nødvendige miljøvariabler:
   ...
   Sæt dem mens du kører alphacode.
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem alphacode, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [alphacode-helicone-session](https://github.com/H2Shami/alphacode-helicone-session) plugin for automatisk at logge hver AlphaCode-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver AlphaCode-samtale opført som en separat session.
...
Du kan konfigurere alphacode til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere alphacode til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere alphacode til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for AlphaCode. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/alphacode) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med AlphaCode:
...
3. Kopier API-nøglen til brug i AlphaCode.
...
6. **Vigtigt**: Før du bruger skymodeller i AlphaCode, skal du hente modelinformationen lokalt:
...
AlphaCode Zen er en liste over testede og verificerede modeller leveret af AlphaCode-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>AlphaCode Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **AlphaCode Zen**.

   ```txt
   /connect
   ```

3. Indtast AlphaCode API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem alphacode-konfigurationen.
...
5. Du kan også tilpasse dem gennem alphacode-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med AlphaCode:
...
Du kan også tilpasse modeller gennem alphacode-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med alphacode. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `alphacode.json`-filen i projektkataloget:
...
`limit`-felterne lader AlphaCode forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `alphacode auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek alphacode-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i alphacode-konfigurationen.
