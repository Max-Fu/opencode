...
Move the canonical AlphaCode terminal application from
`packages/alphacode/src/cli/cmd/tui` into a self-contained workspace package while
...
```text
packages/alphacode ---\
                      > @alphacode-ai/tui -> @alphacode-ai/sdk
packages/cli --------/
```
...
generic presentation libraries. It must not depend on `packages/alphacode`,
`packages/cli`, or `@alphacode-ai/core`.

The SDK is the TUI's AlphaCode boundary. Missing backend data or operations must
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/alphacode`.
- Do not replace private `packages/alphacode` imports with `@alphacode-ai/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@alphacode-ai/tui`.

## Ownership Boundary

### `@alphacode-ai/tui` Owns
...
- AlphaCode domain data displayed by the TUI
...
```text
packages/alphacode/src/cli/cmd/tui
```

Its private dependency on `packages/alphacode` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/alphacode/src/*`.
...
Status: Completed. The private `@alphacode-ai/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@alphacode-ai/tui`.
...
- The package has no dependency on `alphacode`, `@alphacode-ai/cli`, or
  `@alphacode-ai/core`.
...
primitives now live in `@alphacode-ai/tui`. The legacy host consumes explicit
...
and build inputs through `@alphacode-ai/tui/runtime`. Movable app, component,
route, and feature-plugin code no longer reads AlphaCode globals or process state;
...
Replace process-global AlphaCode state with resolved TUI inputs.
...
`@alphacode-ai/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/alphacode` initially.
...
canonical in `@alphacode-ai/tui`. Configured references resolve through the new
...
adapters rather than AlphaCode domain implementations.

Make the SDK the only AlphaCode domain boundary used by the TUI.
...
- Domain-facing TUI code imports AlphaCode data and operations only from
  `@alphacode-ai/sdk`.
...
`@alphacode-ai/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/alphacode`.
...
- The package has no imports from `packages/alphacode`, `packages/cli`, or
  `@alphacode-ai/core`.
...
public `@alphacode-ai/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/alphacode`.
- Keep the legacy embedded worker and server startup in `packages/alphacode`.
- Change those adapters to load config, create transport inputs, and call the
  public `@alphacode-ai/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/alphacode`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@alphacode-ai/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@alphacode-ai/core`, or either
  executable package.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for AlphaCode domain state.
...
cd packages/alphacode && bun typecheck
...
rg '@alphacode-ai/core|packages/alphacode|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/alphacode/src packages/alphacode/test
```
...
- Build the current-platform `packages/alphacode` binary.
