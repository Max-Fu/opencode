    ...
    const msgs = [
      ...
      { role: "assistant", content: text("assistant string") },
      {
        ...
        content: [
          ...
          { type: "tool-call", toolCallId: "call-1", toolName: "Read", input: { filePath: ".alphacode/tool/emoji.ts" } },
          {
            type: "tool-result",
            toolCallId: "call-2",
            toolName: "Read",
...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves metadata using providerID key when store is false", () => {
    const alphacodeModel = {
      ...openaiModel,
      providerID: "alphacode",
      api: {
        id: "alphacode-test",
        url: "https://api.alphacode.ai",
        npm: "@ai-sdk/openai-compatible",
      },
    }
    const msgs = [
    ...
    const result = ProviderTransform.message(msgs, alphacodeModel, { store: false }) as any[]

    expect(result[0].content[0].providerOptions?.alphacode?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.alphacode?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
    ...
    const msgs = [
      {
        ...
        providerOptions: {
          ...
          alphacode: { itemId: "msg_alphacode" },
        ...
        content: [
          {
            ...
            providerOptions: {
              ...
              alphacode: { itemId: "msg_alphacode_part" },
    ...
    expect(result[0].providerOptions?.alphacode?.itemId).toBe("msg_alphacode")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.alphacode?.itemId).toBe("msg_alphacode_part")
