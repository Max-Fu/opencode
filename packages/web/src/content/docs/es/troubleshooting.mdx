...
Para depurar problemas con AlphaCode, comience verificando los registros y los datos locales que almacena en el disco.
...
- **macOS/Linux**: `~/.local/share/alphacode/log/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\alphacode\log`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `alphacode --log-level DEBUG`.

---

## Almacenamiento

alphacode almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/alphacode/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\alphacode`
...
AlphaCode Desktop ejecuta un servidor AlphaCode local (el sidecar `alphacode-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `AlphaCode` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux**: `~/.config/alphacode/alphacode.jsonc` (o `~/.config/alphacode/alphacode.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/alphacode/alphacode.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\alphacode\alphacode.jsonc`
...
AlphaCode también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:

- **Complementos globales**
  - **macOS/Linux**: `~/.config/alphacode/plugins/`
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\alphacode\plugins`
- **Complementos de proyecto** (solo si usas la configuración por proyecto)
  - `<your-project>/.alphacode/plugins/`
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que AlphaCode pueda reconstruirlo.

1. Salga de AlphaCode Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/alphacode`
- **Linux**: eliminar `~/.cache/alphacode` (o ejecutar `rm -rf ~/.cache/alphacode`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\alphacode`

3. Reinicie el escritorio AlphaCode.

---

### Solucionar problemas de conexión del servidor

AlphaCode Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `alphacode.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.

#### Verificar variables de entorno

Si tiene `ALPHACODE_PORT` configurado en su entorno, la aplicación de escritorio intentará usar ese puerto para el servidor local.

- Desarme `ALPHACODE_PORT` (o elija un puerto libre) y reinicie.
...
En Windows, el escritorio AlphaCode requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de AlphaCode.

---

### Notificaciones que no se muestran

AlphaCode Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para AlphaCode en la configuración de su sistema operativo, y
...
1. Salga del escritorio AlphaCode.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio AlphaCode):

- `alphacode.settings.dat` (URL del servidor predeterminado de escritorio)
- `alphacode.global.dat` y `alphacode.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
Si tiene problemas con AlphaCode:
...
1. **Informar problemas el GitHub**
   ...
   [**github.com/anomalyco/alphacode/issues**](https://github.com/anomalyco/alphacode/issues)
...
2. **Únete a nuestro Discord**
   ...
   [**alphacode.ai/discord**](https://alphacode.ai/discord)
...
3. Asegúrese de tener la última versión con `alphacode upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `alphacode models`
...
2. Si el problema persiste, intente borrar la configuración almacenada:
   ...
   ```bash
   rm -rf ~/.local/share/alphacode
   ```

   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.local\share\alphacode`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. alphacode instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   ```bash
   rm -rf ~/.cache/alphacode
   ```

   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\alphacode`

2. Reinicie alphacode para reinstalar los paquetes del proveedor más recientes.

Esto obligará a alphacode a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
alphacode detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
