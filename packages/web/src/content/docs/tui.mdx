---
title: TUI
description: Using the AlphaCode terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

AlphaCode provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running AlphaCode starts the TUI for the current directory.
...
When using the AlphaCode TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to AlphaCode. Allows you to select from available providers and add their API keys.
...
Exit AlphaCode. _Aliases_: `/quit`, `/q`
...
This is separate from `alphacode.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long AlphaCode waits after the leader key. Defaults to `2000`.
...
Use `ALPHACODE_TUI_CONFIG` to load a custom TUI config path.
...
- `sound_pack` - Sound pack ID to use. Defaults to `alphacode.default`.
