...
import { GoogleVertexPlugin } from "@alphacode-ai/core/plugin/provider/google-vertex"
      ...
      yield* catalog.transform((catalog) =>
        catalog.provider.update(ProviderV2.ID.alphacode, (provider) => {
          provider.api = {
            ...
            url: "https://alphacode.ai/zen/v1",
          }
        }),
      )
      yield* addPlugin()

      const provider = required(yield* catalog.provider.get(ProviderV2.ID.alphacode))
