...
Wenn AlphaCode Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\alphacode\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `alphacode --log-level DEBUG`.

---

## Speicher

alphacode speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/alphacode/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\alphacode`
...
AlphaCode Desktop startet im Hintergrund einen lokalen AlphaCode-Server (`alphacode-cli`-Sidecar).
...
- Nur macOS: `AlphaCode`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/alphacode/alphacode.jsonc` (oder `~/.config/alphacode/alphacode.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/alphacode/alphacode.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\alphacode\alphacode.jsonc`
...
AlphaCode kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/alphacode/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\alphacode\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit AlphaCode ihn neu aufbauen kann.

1. Beende AlphaCode Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/alphacode`
- **Linux**: loesche `~/.cache/alphacode` (oder fuehre aus: `rm -rf ~/.cache/alphacode`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\alphacode`

3. Starte AlphaCode Desktop neu.

---

### Server-Verbindungsprobleme beheben

AlphaCode Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `alphacode.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `ALPHACODE_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `ALPHACODE_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt AlphaCode Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit AlphaCode-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

AlphaCode Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer AlphaCode in den OS-Einstellungen aktiviert sind, und
...
1. Beende AlphaCode Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von AlphaCode Desktop):

- `alphacode.settings.dat` (desktop default server URL)
- `alphacode.global.dat` und `alphacode.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit AlphaCode hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `alphacode models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\alphacode`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. alphacode installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\alphacode`

2. Starte alphacode neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt alphacode, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
alphacode erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
