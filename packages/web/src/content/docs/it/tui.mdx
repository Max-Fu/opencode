---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di AlphaCode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

AlphaCode fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire AlphaCode avvia la TUI per la directory corrente.
...
Quando usi la TUI di AlphaCode, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a AlphaCode. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da AlphaCode. _Alias_: `/quit`, `/q`
...
Questo è separato da `alphacode.json`, che configura il comportamento di server/runtime.
...
- `leader_timeout` - Controlla per quanto tempo AlphaCode attende dopo il leader key. Default: `2000`.
...
- `sound_pack` - ID del sound pack da usare. Default: `alphacode.default`.
