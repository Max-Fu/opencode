...
As habilidades do agente permitem que o alphacode descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O alphacode pesquisa nesses locais:

- Configuração do projeto: `.alphacode/skills/<name>/SKILL.md`
- Configuração global: `~/.config/alphacode/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o alphacode sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.alphacode/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/alphacode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.alphacode/skills/git-release/SKILL.md` assim:
...
O alphacode lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `alphacode.json`:
...
**Para agentes embutidos** (em `alphacode.json`):
