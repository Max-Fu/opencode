...
it.instance("plan agent denies edits except .alphacode/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".alphacode/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".alphacode", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.ALPHACODE_TEST_HOME
      process.env.ALPHACODE_TEST_HOME = test.directory
