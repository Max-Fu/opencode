---
title: Servidor
description: Interactuar con el servidor alphacode a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `alphacode serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente alphacode puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `alphacode.local` |
...
Configure `ALPHACODE_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `alphacode`, o configure `ALPHACODE_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `alphacode serve` como a `alphacode web`.
...
Cuando ejecuta `alphacode`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor alphacode para interactuar con alphacode mediante programación.
:::

Esta arquitectura permite que alphacode admita múltiples clientes y le permite interactuar con alphacode mediante programación.

Puede ejecutar `alphacode serve` para iniciar un servidor independiente. Si tienes el
alphacode TUI ejecutándose, `alphacode serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos AlphaCode [IDE](/docs/ide).
...
El servidor alphacode expone las siguientes API.
