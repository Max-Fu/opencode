...
This is the living design and status document for `@alphacode-ai/codemode` and its existing V2 AlphaCode adapter.
...
### V2 AlphaCode adapter

This section describes the `v2` branch integration. On `dev`, CodeMode is integrated through
`packages/alphacode/src/tool/code-mode.ts`, where nested MCP calls run the `tool.execute.before` and
