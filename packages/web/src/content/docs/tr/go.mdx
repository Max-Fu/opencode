...
AlphaCode Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

Go, AlphaCode'daki diğer sağlayıcılar gibi çalışır. AlphaCode Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve AlphaCode'u kullanmak için bunu kullanmanıza gerek yoktur.
...
AlphaCode Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

AlphaCode Go, AlphaCode'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>AlphaCode Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `AlphaCode Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye AlphaCode Go'ya abone olabilir.
:::
...
AlphaCode Go aşağıdaki limitleri içerir:
...
AlphaCode yapılandırmanızdaki [model id](/docs/config/#models) formatı `alphacode-go/<model-id>` şeklindedir. Örneğin, Kimi K3 için yapılandırmanızda `alphacode-go/kimi-k3` kullanmalısınız.
...
AlphaCode Go'yu şu amaçlarla oluşturduk:
...
4. AlphaCode ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
