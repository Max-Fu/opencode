...
description: Skriv dine egne programtillegg for å utvide AlphaCode.
---

Plugins lar deg utvide AlphaCode ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til AlphaCode.
...
- `.alphacode/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/alphacode/node_modules/`.
...
1. Global konfigurasjon (`~/.config/alphacode/alphacode.json`)
2. Prosjektkonfigurasjon (`alphacode.json`)
3. Global plugin-katalog (`~/.config/alphacode/plugins/`)
4. Prosjektpluginkatalog (`.alphacode/plugins/`)
...
AlphaCode kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En AlphaCode SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide AlphaCode.
...
:::note
Hvis du bruker AlphaCode-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre AlphaCode fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til AlphaCode:
...
`tool`-hjelperen lager et tilpasset verktøy som AlphaCode kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for AlphaCode sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://alphacode.ai/docs/sdk) for detaljer.
