...
describe("tui thread", () => {
  ...
  cliIt.live("rejects mini-only options without --mini", ({ alphacode }) =>
    Effect.gen(function* () {
      const result = yield* alphacode.spawn(["--replay-limit", "10"])

      alphacode.expectExit(result, 1)
      expect(result.stderr).toContain("--replay-limit requires --mini")
    }),
  )

  cliIt.live("routes attached sessions to mini mode", ({ alphacode }) =>
    Effect.gen(function* () {
      const result = yield* alphacode.spawn(["attach", "http://127.0.0.1:1", "--mini"])
  ...
  cliIt.live("rejects network options in mini mode", ({ alphacode }) =>
    Effect.gen(function* () {
      const result = yield* alphacode.spawn(["--mini", "--port", "4096"])
