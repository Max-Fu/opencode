...
تتيح مهارات الوكيل لـ AlphaCode اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث AlphaCode في هذه المواقع:

- إعدادات المشروع: `.alphacode/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/alphacode/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج AlphaCode صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.alphacode/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/alphacode/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.alphacode/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض AlphaCode المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `alphacode.json`:
...
**للوكلاء المدمجين** (في `alphacode.json`):
