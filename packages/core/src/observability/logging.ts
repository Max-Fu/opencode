...
export function fileLogger(file = path.join(Global.Path.log, "alphacode.log"), id: string = runID) {
...
export function minimumLogLevel() {
  const value = process.env.ALPHACODE_LOG_LEVEL?.toUpperCase()
  const levels = {
    DEBUG: "Debug",
    INFO: "Info",
    WARN: "Warn",
...
export function loggers() {
  return process.env.ALPHACODE_PRINT_LOGS === "1" ? [fileLogger(), stderrLogger] : [fileLogger()]
}
