...
Com o alphacode, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o alphacode usa nosso próprio tema `alphacode`.
...
O alphacode vem com vários temas embutidos.
...
- Querem que o alphacode corresponda à aparência do seu terminal
...
O alphacode suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/alphacode/themes/*.json` ou `$XDG_CONFIG_HOME/alphacode/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.alphacode/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.alphacode/themes/*.json`
