...
import { AlphaCodeHttpApi } from "./api"
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for alphacode serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(AlphaCodeHttpApi)
...
export const PublicApi = AlphaCodeHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "alphacode",
    version: "1.0.0",
    description: "alphacode api",
