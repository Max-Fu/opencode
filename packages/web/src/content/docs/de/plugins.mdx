---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um AlphaCode zu erweitern.
---

Mit Plugins können Sie AlphaCode erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von AlphaCode zu ändern.
...
- `.alphacode/plugins/` – Plugins auf Projektebene
- `~/.config/alphacode/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/alphacode/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/alphacode/alphacode.json`)
2. Projektkonfiguration (`alphacode.json`)
3. Globales Plugin-Verzeichnis (`~/.config/alphacode/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.alphacode/plugins/`)
...
AlphaCode führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein AlphaCode-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie AlphaCode erweitern können.
...
:::note
Wenn Sie die AlphaCode-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass AlphaCode `.env`-Dateien liest:
...
Plugins können AlphaCode auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das AlphaCode aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für AlphaCode zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://alphacode.ai/docs/sdk).
