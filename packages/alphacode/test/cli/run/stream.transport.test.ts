...
import { AlphacodeClient, type GlobalEvent } from "@alphacode-ai/sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, LocalReplayRow, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<AlphacodeClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<AlphacodeClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<AlphacodeClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<AlphacodeClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<AlphacodeClient["session"]["status"]>>["data"]>
...
function ok<T>(data: T) {
  return Promise.resolve({
    data,
    error: undefined,
    request: new Request("https://alphacode.test"),
...
function sdk(
  input: {
    ...
    subscribe?: AlphacodeClient["event"]["subscribe"]
    globalEvent?: AlphacodeClient["global"]["event"]
    promptAsync?: AlphacodeClient["session"]["promptAsync"]
    status?: AlphacodeClient["session"]["status"]
    messages?: AlphacodeClient["session"]["messages"]
    children?: AlphacodeClient["session"]["children"]
    permissions?: AlphacodeClient["permission"]["list"]
    questions?: AlphacodeClient["question"]["list"]
  } = {},
) {
  const client = new AlphacodeClient()

  const subscribe: AlphacodeClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: AlphacodeClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: AlphacodeClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: AlphacodeClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: AlphacodeClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: AlphacodeClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: AlphacodeClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: AlphacodeClient["question"]["list"] = input.questions ?? (() => ok([]))
