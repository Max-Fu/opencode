...
export function registerWslIpcHandlers(controller: WslServersController) {
  ...
  ipcMain.handle("wsl-servers-install-alphacode", (_event: IpcMainInvokeEvent, name: string) =>
    controller.installAlphacode(requireWslIpcString("distro", name)),
  )
...
function registerUnavailableWslIpcHandlers() {
  ...
  ipcMain.handle("wsl-servers-install-alphacode", unavailable)
