...
Create `.alphacode/commands/test.md`:
...
You can add custom commands through the AlphaCode config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your AlphaCode [config](/docs/config):
...
- Per-project: `.alphacode/commands/`
...
alphacode includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
