---
title: Tjener
description: Samhandle med AlphaCode-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `alphacode serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en AlphaCode-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `alphacode.local` |
...
Sett `ALPHACODE_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `alphacode` som standard, eller sett `ALPHACODE_SERVER_USERNAME` for å overstyre det. Dette gjelder både `alphacode serve` og `alphacode web`.
...
Når du kjører `alphacode` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk AlphaCode-serveren til å samhandle med AlphaCode programmatisk.
:::

Denne arkitekturen lar AlphaCode støtte flere klienter og lar deg samhandle med AlphaCode programmatisk.

Du kan kjøre `alphacode serve` for å starte en frittstående server. Hvis du har
AlphaCode TUI kjører, vil `alphacode serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av AlphaCode [IDE](/docs/ide) plugins.
...
alphacode-serveren viser følgende APIer.
