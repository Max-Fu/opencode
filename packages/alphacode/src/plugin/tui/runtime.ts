...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@alphacode-ai/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { AppNodeBuilder } from "@alphacode-ai/core/effect/app-node-builder"
import { errorData, errorMessage } from "@alphacode-ai/tui/util/error"
...
import { hasTheme, upsertTheme } from "@alphacode-ai/tui/context/theme"
import { Global } from "@alphacode-ai/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@alphacode-ai/core/util/flock"
import { Flag } from "@alphacode-ai/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@alphacode-ai/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@alphacode-ai/core/v1/config/plugin"
import { createCommandShim } from "@alphacode-ai/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@alphacode-ai/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".alphacode"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".alphacode", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".alphacode", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.ALPHACODE_PURE ? [] : pluginOrigins
    if (Flag.ALPHACODE_PURE && pluginOrigins.length) {
    }
