...
Delete `packages/alphacode/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/alphacode/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/alphacode/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/alphacode`
- TUI plugin loader tests in `packages/alphacode` if runtime plugin API wiring changed
