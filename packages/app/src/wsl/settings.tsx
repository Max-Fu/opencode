...
import { wslAlphacodeAction, wslRuntimeRetryable } from "./settings-model"
    ...
    <Show when={api}>
      <For each={props.servers()}>
        {(item) => {
          ...
          const check = () => wsl.data?.alphacodeChecks[item.config.distro]
          const alphacodeAction = () => wslAlphacodeAction(check())
          const busy = () => wsl.data?.job?.kind === "install-alphacode" && wsl.data.job.distro === item.config.distro
          return (
            <div class="settings-v2-servers-row">
              ...
              <div class="settings-v2-servers-actions">
                ...
                <Show when={alphacodeAction()}>
                  {(label) => (
                    <ButtonV2
                      size="small"
                      disabled={busy() || request.isPending}
                      onClick={() => api && request.mutate(() => api.installAlphacode(item.config.distro))}
                    >
