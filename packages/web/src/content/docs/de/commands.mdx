...
Erstellen Sie `.alphacode/commands/test.md`:

```md title=".alphacode/commands/test.md"
---
...
Sie können benutzerdefinierte Befehle über die AlphaCode-Konfiguration hinzufügen oder indem Sie Markdown-Dateien im Verzeichnis `commands/` erstellen.

---

### JSON

Verwenden Sie die Option `command` in Ihrem AlphaCode [config](/docs/config):

```json title="alphacode.jsonc" {4-12}
{
...
- Global: `~/.config/alphacode/commands/`
- Pro Projekt: `.alphacode/commands/`

```markdown title="~/.config/alphacode/commands/test.md"
---
description: Run tests with coverage
agent: build
model: anthropic/claude-3-5-sonnet-20241022
...
```md title=".alphacode/commands/component.md"
---
...
```md title=".alphacode/commands/create-file.md"
---
...
```md title=".alphacode/commands/analyze-coverage.md"
---
...
```md title=".alphacode/commands/review-changes.md"
---
...
```md title=".alphacode/commands/review-component.md"
---
...
AlphaCode enthält mehrere integrierte Befehle wie `/init`, `/undo`, `/redo`, `/share`, `/help`; [mehr erfahren](/docs/tui#commands).
