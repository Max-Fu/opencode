---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u AlphaCode.
---

import config from "../../../../config.mjs"
export const console = config.console

AlphaCode koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj AlphaCode konfiguraciji.
...
u `~/.local/share/alphacode/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem AlphaCode
config.
...
AlphaCode Zen je lista modela koje je obezbedio AlphaCode tim koji su bili
testirano i potvrđeno da dobro radi sa AlphaCode. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa AlphaCode Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite alphacode i idite na [alphacode.ai/auth](https://alphacode.ai/auth).
...
Radi kao i svaki drugi provajder u AlphaCode i potpuno je opcionalan za korištenje.

---

## AlphaCode Go

AlphaCode Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža AlphaCode tim i koji su testirani i verificirani da dobro rade s AlphaCode-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `AlphaCode Go` i idite na [alphacode.ai/auth](https://alphacode.ai/zen).
...
Da biste koristili Amazon Bedrock s AlphaCode:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete alphacode:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `alphacode.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u AlphaCode.
:::
...
Možete konfigurirati alphacode za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi alphacode ispravno radio.
   :::
...
Također možete dodati modele kroz svoju alphacode konfiguraciju.
...
da pohrani token u alphacode auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
AlphaCode koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate AlphaCode
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`alphacode.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [alphacode-gitlab-auth](https://www.npmjs.com/package/alphacode-gitlab-auth).

##### Konfiguracija

Prilagodite putem `alphacode.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s alphacode:
...
Za korištenje Google Vertex AI s AlphaCode:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće alphacode.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem alphacodea, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [alphacode-helicone-session](https://github.com/H2Shami/alphacode-helicone-session) da automatski prijavite svaki AlphaCode razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki AlphaCode razgovor naveden kao zasebna sesija.
...
Možete konfigurirati alphacode za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati alphacode za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati alphacode za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za AlphaCode. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/alphacode) za detalje.
:::
...
Da biste koristili Ollama Cloud s AlphaCode:
...
3. Kopirajte API ključ za korištenje u AlphaCode.
...
6. **Važno**: Prije upotrebe modela oblaka u AlphaCode, morate lokalno povući informacije o modelu:
...
AlphaCode Zen je lista testiranih i verifikovanih modela koju je obezbedio AlphaCode tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>AlphaCode Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **AlphaCode Zen**.

```txt
   /connect
```

3. Unesite svoj AlphaCode API ključ.
...
Također možete dodati dodatne modele putem vaše alphacode konfiguracije.
...
5. Također ih možete prilagoditi putem vaše alphacode konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa Alphacodeom:
...
Također možete prilagoditi modele kroz svoju alphacode konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s alphacode-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `alphacode.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju AlphaCode da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `alphacode auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite AlphaCode konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj alphacode konfiguraciji.
