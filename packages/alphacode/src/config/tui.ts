...
import { isRecord } from "@alphacode-ai/tui/util/record"
...
import { TuiKeybind } from "@alphacode-ai/tui/config/keybind"
...
import { TuiConfig } from "@alphacode-ai/tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@alphacode/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old alphacode.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.alphacode`
  // folders between cwd and home, and ALPHACODE_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.ALPHACODE_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit ALPHACODE_TUI_CONFIG override, if set.
  if (Flag.ALPHACODE_TUI_CONFIG) {
    const configFile = Flag.ALPHACODE_TUI_CONFIG
  ...
  // 4. `.alphacode` directories (and ALPHACODE_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".alphacode") || dir === Flag.ALPHACODE_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".alphacode") && dir !== Flag.ALPHACODE_CONFIG_DIR) continue
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    // Opt-in; see ALPHACODE_ENABLE_PLUGIN_DEP_INSTALL. Skipped entirely by
    // default so startup makes no registry request.
    const deps = Flag.ALPHACODE_ENABLE_PLUGIN_DEP_INSTALL
