---
title: Konfiguracija
description: Korištenje AlphaCode JSON konfiguracije.
---

Možete konfigurirati AlphaCode koristeći JSON konfiguracijski fajl.

---

## Format

AlphaCode podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/alphacode`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/alphacode/alphacode.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`ALPHACODE_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`alphacode.json` u projektu) - postavke specifične za projekat
5. **`.alphacode` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`ALPHACODE_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.alphacode` i `~/.config/alphacode` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/alphacode` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu AlphaCode konfiguraciju u `~/.config/alphacode/alphacode.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/alphacode/tui.json`.
...
Dodajte `alphacode.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se AlphaCode pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `ALPHACODE_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `ALPHACODE_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.alphacode` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.alphacode`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`alphacode.ai/config.json`**](https://alphacode.ai/config.json).

TUI konfiguracija koristi [**`alphacode.ai/tui.json`**](https://alphacode.ai/tui.json).
...
Koristite `ALPHACODE_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `alphacode.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `alphacode serve` i `alphacode web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš AlphaCode server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `alphacode.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj AlphaCode konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, AlphaCode pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/alphacode/agents/` ili `.alphacode/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, AlphaCode će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`alphacode run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/alphacode/commands/` ili `.alphacode/commands/`. [Saznajte više ovdje](/docs/commands).
...
AlphaCode će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, AlphaCode **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju AlphaCode sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.alphacode/plugins/` ili `~/.config/alphacode/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite AlphaCode da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
