...
import type { LLMClientShape } from "@alphacode-ai/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("alphacode"))
    return { type: "unsupported", reason: "provider is not openai, alphacode, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @alphacode-ai/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains alphacode-owned. The native runtime only adapts
      // the @alphacode-ai/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
