...
describe("inference stat normalization", () => {
  ...
  test("uses provider.model to resolve alphacode route providers", () => {
    ...
    expect(statProvider("big-pickle", "claude-sonnet-4-5", "alphacode")).toBe("anthropic")
    expect(statProvider("big-pickle", "gpt-5", "alphacode")).toBe("openai")
    expect(statProvider("big-pickle", "", "alphacode")).toBe("unknown")
  ...
  test("model aggregates prefer provider.model and use normalized model", () => {
    ...
    expect(
      toModelAggregate({ ...aggregate("big-pickle", "alphacode"), provider_model: "claude-sonnet-4-5" }),
    ).toMatchObject([
      {
        provider: "anthropic",
  ...
  test("provider aggregates never keep alphacode as the provider", () => {
    expect(toProviderAggregate({ ...aggregate("big-pickle", "alphacode"), provider_model: "gpt-5" })).toMatchObject([
      { provider: "openai" },
    ])
    expect(toProviderAggregate(aggregate("big-pickle", "alphacode"))).toMatchObject([{ provider: "unknown" }])
  })

  test("geo aggregates never keep alphacode or big-pickle dimensions", () => {
    expect(toGeoAggregate({ ...aggregate("big-pickle", "alphacode"), country: "US" })).toMatchObject([
