...
function requestServer(path: string, directory: string, init: RequestInit = {}) {
  ...
  headers.set("x-alphacode-directory", directory)
      ...
      try {
        const response = yield* requestDefault(url.toString(), dir, {
          ...
          headers: {
            "accept-encoding": "br",
            "content-type": "application/json",
            "x-alphacode-workspace": "internal",
          },
          body: JSON.stringify({ $schema: "https://alphacode.ai/config.json" }),
        })
        ...
        expect(forwarded[0]?.headers).not.toHaveProperty("x-alphacode-directory")
        expect(forwarded[0]?.headers).not.toHaveProperty("x-alphacode-workspace")
