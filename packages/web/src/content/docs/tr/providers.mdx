---
title: Sağlayıcılar
description: alphacode'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

alphacode, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı alphacode yapılandırmanızda yapılandırın.
...
`~/.local/share/alphacode/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları alphacode'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
AlphaCode Zen, alphacode ekibi tarafından sağlanan modellerin bir listesidir.
alphacode ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz AlphaCode Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, alphacode'u seçin ve [alphacode.ai/auth](https://alphacode.ai/auth)'ye gidin.
...
alphacode'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## AlphaCode Go

AlphaCode Go, alphacode ile iyi çalıştığı test edilmiş ve doğrulanmış, alphacode ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `AlphaCode Go`'yu seçin ve [alphacode.ai/auth](https://alphacode.ai/zen) adresine gidin.
...
Amazon Bedrock'u alphacode ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   alphacode'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `alphacode.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi alphacode'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
alphacode'u, yerel LLM'leri OpenAI uyumlu bir API sunucusunun arkasında çalıştıran bir masaüstü uygulaması olan [Atomic Chat](https://atomic.chat) aracılığıyla yerel modelleri kullanacak şekilde yapılandırabilirsiniz (varsayılan uç nokta `http://127.0.0.1:1337/v1`).
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   alphacode'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
5. Bir model seçmek için `/models` komutunu çalıştırın.
   ...
   Alphacode yapılandırmanız aracılığıyla da modeller ekleyebilirsiniz.
...
:::note[uyumluluk notu]
alphacode, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. alphacode'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`alphacode.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
Daha fazla belge [alphacode-gitlab-auth](https://www.npmjs.com/package/alphacode-gitlab-auth) ana sayfasında.

##### Yapılandırma

`alphacode.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi alphacode'la kullanmak için:
...
Google Vertex AI'yi alphacode ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları alphacode çalıştırılırken ayarlayın.
...
Helicone'un alphacode aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her alphacode görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [alphacode-helicone-session](https://github.com/H2Shami/alphacode-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her alphacode görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
alphacode'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla alphacode'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla alphacode'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini alphacode için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/alphacode)'a bakın.
:::
...
Ollama Cloud'u alphacode ile kullanmak için:
...
3. alphacode'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: alphacode'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
AlphaCode Zen, alphacode ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>AlphaCode Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **AlphaCode Zen**'i arayın.

   ```txt
   /connect
   ```

3. alphacode API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   alphacode yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca alphacode yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
[Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/)'yi Alphacode ile kullanmak için:
...
Modelleri alphacode yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı alphacode ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `alphacode.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları alphacode'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `alphacode auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için alphacode yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, alphacode yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
