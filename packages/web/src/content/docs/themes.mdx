...
With AlphaCode you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, AlphaCode uses our own `alphacode` theme.
...
AlphaCode comes with several built-in themes.
...
- Want AlphaCode to match their terminal's appearance
...
AlphaCode supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/alphacode/themes/*.json` or `$XDG_CONFIG_HOME/alphacode/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.alphacode/themes/*.json`
4. **Current working directory** - Defined in `./.alphacode/themes/*.json`
