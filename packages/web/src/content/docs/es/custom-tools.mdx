...
description: Cree herramientas que LLM pueda llamar en alphacode.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de alphacode como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.alphacode/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/alphacode/tools/`.
...
```ts title=".alphacode/tools/database.ts" {1}
...
```ts title=".alphacode/tools/math.ts"
import { tool } from "@alphacode-ai/plugin"
...
```ts title=".alphacode/tools/bash.ts"
import { tool } from "@alphacode-ai/plugin"
...
```ts title=".alphacode/tools/project.ts" {8}
import { tool } from "@alphacode-ai/plugin"
...
```python title=".alphacode/tools/add.py"
import sys
...
```ts title=".alphacode/tools/python-add.ts" {10}
import { tool } from "@alphacode-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".alphacode/tools/add.py")
