...
describe("alphacode read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // ALPHACODE_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ alphacode }) =>
      Effect.gen(function* () {
        const r = yield* alphacode.spawn(["mcp", "list"])
        alphacode.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ alphacode }) =>
      Effect.gen(function* () {
        const r = yield* alphacode.spawn(["providers", "list"])
        alphacode.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ alphacode }) =>
      Effect.gen(function* () {
        const r = yield* alphacode.spawn(["models"])
        alphacode.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ alphacode }) =>
      Effect.gen(function* () {
        const r = yield* alphacode.spawn(["agent", "list"])
        alphacode.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh ALPHACODE_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ alphacode }) =>
      Effect.gen(function* () {
        const r = yield* alphacode.spawn(["session", "list"])
        alphacode.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ alphacode }) =>
      Effect.gen(function* () {
        const r = yield* alphacode.spawn(["stats"])
        alphacode.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under ALPHACODE_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ alphacode }) =>
      Effect.gen(function* () {
        const r = yield* alphacode.spawn(["db", "path"])
        alphacode.expectExit(r, 0, "db path")
