---
title: Configuration
description: Utilisation de la configuration JSON AlphaCode.
---

Vous pouvez configurer AlphaCode à l'aide d'un fichier de configuration JSON.

---

## Format

AlphaCode prend en charge les formats **JSON** et **JSONC** (JSON avec commentaires).
...
1. **Configuration distante** (à partir de `.well-known/alphacode`) - paramètres par défaut de l'organisation
2. **Configuration globale** (`~/.config/alphacode/alphacode.json`) - préférences utilisateur
3. **Configuration personnalisée** (`ALPHACODE_CONFIG` env var) - remplacements personnalisés
4. **Configuration du projet** (`alphacode.json` dans le projet) - paramètres spécifiques au projet
5. **Répertoires `.alphacode`** - agents, commandes, plugins
6. **Configuration en ligne** (`ALPHACODE_CONFIG_CONTENT` env var) - remplacements d'exécution
...
:::note
Les répertoires `.alphacode` et `~/.config/alphacode` utilisent des **noms au pluriel** pour les sous-répertoires : `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` et `themes/`. Les noms singuliers (par exemple, `agent/`) sont également pris en charge pour une compatibilité ascendante.
:::
...
Les organisations peuvent fournir une configuration par défaut via le point de terminaison `.well-known/alphacode`. Ceci est récupéré automatiquement lorsque vous vous authentifiez auprès d’un fournisseur qui le prend en charge.
...
Placez votre configuration globale AlphaCode dans `~/.config/alphacode/alphacode.json`. Utilisez la configuration globale pour les préférences de l'utilisateur telles que les fournisseurs, les modèles et les autorisations.

Pour les paramètres spécifiques à TUI, utilisez `~/.config/alphacode/tui.json`.
...
Ajoutez `alphacode.json` à la racine de votre projet. La configuration du projet a la priorité la plus élevée parmi les fichiers de configuration standard : elle remplace les configurations globales et distantes.
...
Lorsque AlphaCode démarre, il recherche un fichier de configuration dans le répertoire actuel ou remonte jusqu'au répertoire Git le plus proche.
...
Spécifiez un chemin de fichier de configuration personnalisé à l'aide de la variable d'environnement `ALPHACODE_CONFIG`.
...
Spécifiez un répertoire de configuration personnalisé à l'aide de `ALPHACODE_CONFIG_DIR` variable d'environnement. Ce répertoire sera recherché pour les agents, les commandes, modes et plugins tout comme le répertoire standard `.alphacode`, et devrait suivre la même structure.
...
Le répertoire personnalisé est chargé après les répertoires de configuration globale et `.alphacode`, il **peut donc remplacer** leurs paramètres.

---

## Schéma

Le fichier de configuration a un schéma défini dans [**`alphacode.ai/config.json`**](https://alphacode.ai/config.json).

La configuration TUI utilise [**`alphacode.ai/tui.json`**](https://alphacode.ai/tui.json).
...
Utilisez `ALPHACODE_TUI_CONFIG` pour pointer vers un fichier de configuration TUI personnalisé.

Les anciennes clés `theme`, `keybinds` et `tui` dans `alphacode.json` sont obsolètes et migrées automatiquement lorsque cela est possible.
...
Vous pouvez configurer les paramètres du serveur pour les commandes `alphacode serve` et `alphacode web` via l'option `server`.
...
- `mdns` - Activer la découverte du service mDNS. Cela permet à d'autres appareils du réseau de découvrir votre serveur AlphaCode.
- `mdnsDomain` - Nom de domaine personnalisé pour le service mDNS. La valeur par défaut est `alphacode.local`. Utile pour exécuter plusieurs instances sur le même réseau.
...
Vous pouvez configurer les fournisseurs et les modèles que vous souhaitez utiliser dans votre configuration AlphaCode via les options `provider`, `model` et `small_model`.
...
L'option `small_model` configure un modèle distinct pour les tâches légères comme la génération de titres. Par défaut, AlphaCode essaie d'utiliser un modèle moins cher s'il est disponible auprès de votre fournisseur, sinon il revient à votre modèle principal.
...
Vous pouvez également définir des agents à l'aide de fichiers markdown dans `~/.config/alphacode/agents/` ou `.alphacode/agents/`. [En savoir plus ici](/docs/agents).
...
L'agent par défaut doit être un agent principal (et non un sous-agent). Il peut s'agir d'un agent intégré tel que `"build"` ou `"plan"`, ou d'un [agent personnalisé](/docs/agents) que vous avez défini. Si l'agent spécifié n'existe pas ou est un sous-agent, AlphaCode reviendra à `"build"` avec un avertissement.

Ce paramètre s'applique à toutes les interfaces : TUI, CLI (`alphacode run`), application de bureau et GitHub Action.
...
Vous pouvez également définir des commandes à l'aide de fichiers markdown dans `~/.config/alphacode/commands/` ou `.alphacode/commands/`. [En savoir plus ici](/docs/commands).
...
AlphaCode téléchargera automatiquement toutes les nouvelles mises à jour au démarrage. Vous pouvez désactiver cela avec l'option `autoupdate`.
...
Par défaut, alphacode **autorise toutes les opérations** sans nécessiter d'approbation explicite. Vous pouvez modifier cela en utilisant l'option `permission`.
...
[Plugins](/docs/plugins) étendent AlphaCode avec des outils, des hooks et des intégrations personnalisés.

Placez les fichiers du plugin dans `.alphacode/plugins/` ou `~/.config/alphacode/plugins/`. Vous pouvez également charger des plugins depuis npm via l'option `plugin`.
...
Ceci est utile lorsque vous souhaitez restreindre AlphaCode à l'utilisation de fournisseurs spécifiques plutôt que de les désactiver un par un.
