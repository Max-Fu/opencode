---
title: Server
description: Komunicirajte s alphacode serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `alphacode serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji alphacode klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `alphacode.local` |
...
Postavite `ALPHACODE_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `alphacode`, ili postavite `ALPHACODE_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `alphacode serve` i za `alphacode web`.
...
Kada pokrenete `alphacode`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite alphacode server za programsku interakciju sa alphacode-om.
:::

Ova arhitektura omogucava alphacode podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `alphacode serve` da startate standalone server. Ako je alphacode TUI vec pokrenut, `alphacode serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste AlphaCode [IDE](/docs/ide) pluginovi.
...
alphacode server izlaže sljedece API-je.
