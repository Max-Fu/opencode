...
_Model Bağlam Protokolü_ veya MCP kullanarak alphacode'a harici araçlar ekleyebilirsiniz. alphacode hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [alphacode Config](https://alphacode.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/alphacode` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
alphacode, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde alphacode şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa alphacode, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `alphacode mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından alphacode, belirteçleri `~/.local/share/alphacode/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra alphacode'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi alphacode yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve alphacode'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
