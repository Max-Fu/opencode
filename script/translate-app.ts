...
export function parseTranslationArgs(args: string[]) {
  const parsed = parseArgs({
    ...
    options: {
      ...
      model: { type: "string", default: "alphacode/gpt-5.5" },
...
export function glossaryFile(locale: Locale) {
  if (locale === "zh") return ".alphacode/glossary/zh-cn.md"
  if (locale === "zht") return ".alphacode/glossary/zh-tw.md"
  return `.alphacode/glossary/${locale}.md`
}
...
export function sessionIDFromEvents(output: string) {
  ...
  if (!match?.[1]) throw new Error("AlphaCode did not report a session ID.")
  return match[1]
}

export function sessionModels(value: unknown) {
  if (!isRecord(value) || !Array.isArray(value.messages))
    throw new Error("AlphaCode returned an invalid session export.")
  return value.messages.flatMap((message) => {
    ...
    if (typeof message.info.providerID !== "string" || typeof message.info.modelID !== "string") {
      throw new Error("AlphaCode session export omitted the assistant model.")
    }
    return [
      {
...
Options:
  -c, --concurrency <count>  Maximum parallel AlphaCode runs for 'all' (default: 4)
      --model <provider/id>  AlphaCode model (default: alphacode/gpt-5.5)
      --variant <name>       Model variant (default: xhigh)
      --dry-run              Report drift without running AlphaCode
...
`)
  ...
  if (failed.length) console.error(`\nAlphaCode failed for: ${failed.map((result) => result.locale).join(", ")}`)
...
async function translate(
  ...
  env.ALPHACODE_DISABLE_PROJECT_CONFIG = "1"
  env.ALPHACODE_CONFIG_CONTENT = JSON.stringify(
    translationConfig(
      agent,
      model,
  ...
  const exported = Bun.spawn(["alphacode", "--pure", "export", sessionID, "--sanitize"], {
    cwd: root,
    env,
    stdout: "pipe",
    stderr: "pipe",
...
async function resolveModelVariant(model: string, variant: string) {
  ...
  const env = isolatedEnvironment()
  env.ALPHACODE_DISABLE_PROJECT_CONFIG = "1"
  const proc = Bun.spawn(["alphacode", "--pure", "models", provider, "--verbose"], {
    cwd: root,
    env,
    stdin: "ignore",
    stdout: "pipe",
...
function isolatedEnvironment() {
  ...
  delete env.ALPHACODE_CONFIG
  delete env.ALPHACODE_CONFIG_DIR
  delete env.ALPHACODE_CONFIG_CONTENT
  delete env.ALPHACODE_PERMISSION
  delete env.ALPHACODE_AUTO_SHARE
  return env
}
