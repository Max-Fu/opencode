---
title: Complementos
description: Escriba sus propios complementos para extender AlphaCode.
---

Los complementos le permiten extender AlphaCode al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de AlphaCode.
...
- `.alphacode/plugins/` - Complementos a nivel de proyecto
- `~/.config/alphacode/plugins/` - Complementos globales
...
```json title="alphacode.json"
{
  ...
  "plugin": ["alphacode-helicone-session", "alphacode-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/alphacode/node_modules/`.
...
1. Configuración global (`~/.config/alphacode/alphacode.json`)
2. Configuración del proyecto (`alphacode.json`)
3. Directorio global de complementos (`~/.config/alphacode/plugins/`)
4. Directorio de complementos del proyecto (`.alphacode/plugins/`)
...
```json title=".alphacode/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
AlphaCode ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".alphacode/plugins/my-plugin.ts"
...
```js title=".alphacode/plugins/example.js"
...
- `client`: Un cliente SDK alphacode para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar alphacode.
...
```js title=".alphacode/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "alphacode"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio AlphaCode, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que alphacode lea archivos `.env`:

```javascript title=".alphacode/plugins/env-protection.js"
...
```javascript title=".alphacode/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a alphacode:

```ts title=".alphacode/plugins/custom-tools.ts"
import { type Plugin, tool } from "@alphacode-ai/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que alphacode puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para alphacode junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://alphacode.ai/docs/sdk) para obtener más detalles.
...
```ts title=".alphacode/plugins/compaction.ts"
import type { Plugin } from "@alphacode-ai/plugin"
...
```ts title=".alphacode/plugins/custom-compaction.ts"
import type { Plugin } from "@alphacode-ai/plugin"
