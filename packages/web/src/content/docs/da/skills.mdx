...
Agentferdigheter lar AlphaCode oppdage gjenbrugbare instruksjoner fra repo- eller hjemmekatalogen din.
...
AlphaCode søker etter disse stedene:

- Prosjektkonfigurasjon: `.alphacode/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/alphacode/skills/<name>/SKILL.md`
...
For prosjektlokale stier går AlphaCode opp fra din nåværende arbejdskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.alphacode/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/alphacode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.alphacode/skills/git-release/SKILL.md` slik:
...
AlphaCode viser tilgængelige ferdigheter i `skill` verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved at bruge mønsterbaserte tillatelser i `alphacode.json`:
...
**For innebygde agenter** (i `alphacode.json`):
