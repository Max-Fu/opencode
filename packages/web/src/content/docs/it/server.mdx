---
title: Server
description: Interagisci con il server alphacode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `alphacode serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client alphacode.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `alphacode.local` |
...
Imposta `ALPHACODE_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `alphacode`, oppure imposta `ALPHACODE_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `alphacode serve` sia per `alphacode web`.
...
Quando esegui `alphacode` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server alphacode per interagire con alphacode in modo programmatico.
:::

Questa architettura permette a alphacode di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `alphacode serve` per avviare un server standalone. Se la TUI di alphacode e gia in esecuzione, `alphacode serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di AlphaCode.
...
Il server alphacode espone le seguenti API.
