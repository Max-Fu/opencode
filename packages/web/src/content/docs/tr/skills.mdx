...
Ajan becerileri, alphacode'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
alphacode su konumlari tarar:

- Proje konfigurasyonu: `.alphacode/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/alphacode/skills/<name>/SKILL.md`
...
Proje yerel yollarinda alphacode, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.alphacode/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/alphacode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.alphacode/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
alphacode, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`alphacode.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`alphacode.json` icinde):
