...
export const DEPRECATED_PLUGIN_PACKAGES = ["alphacode-openai-codex-auth", "alphacode-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, alphacodeVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(alphacodeVersion) || semver.major(alphacodeVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.alphacode
  if (typeof range !== "string") return
  if (!semver.satisfies(alphacodeVersion, range)) {
    throw new Error(`Plugin requires alphacode ${range} but running ${alphacodeVersion}`)
  }
}
