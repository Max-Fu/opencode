---
title: Plugins
description: Escreva seus próprios plugins para estender o alphacode.
---

Plugins permitem que você estenda o alphacode conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do alphacode.
...
- `.alphacode/plugins/` - Plugins em nível de projeto
- `~/.config/alphacode/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/alphacode/node_modules/`.
...
1. Configuração global (`~/.config/alphacode/alphacode.json`)
2. Configuração do projeto (`alphacode.json`)
3. Diretório de plugins global (`~/.config/alphacode/plugins/`)
4. Diretório de plugins do projeto (`.alphacode/plugins/`)
...
O alphacode executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do alphacode para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o alphacode.
...
:::note
Se você estiver usando o aplicativo desktop alphacode, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o alphacode de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao alphacode:
...
O helper `tool` cria uma ferramenta personalizada que o alphacode pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o alphacode junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://alphacode.ai/docs/sdk) para detalhes.
