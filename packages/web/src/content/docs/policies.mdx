---
title: Policies
description: Control which configured resources AlphaCode may use.
---

Policies control whether AlphaCode may perform an action on a named resource. This feature is experimental and is configured with the `experimental.policies` array in `alphacode.json`.

Policies are separate from [permissions](/docs/permissions). Permissions control what tools can do during a session, while policies control whether AlphaCode may use a resource such as an LLM provider.
...
AlphaCode currently supports one policy action:
