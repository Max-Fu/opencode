...
Možete dodati vanjske alate u AlphaCode koristeći _Model Context Protocol_, ili MCP. AlphaCode podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [AlphaCode Config](https://alphacode.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/alphacode`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
AlphaCode automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, AlphaCode će:
...
Ako server zahtijeva autentifikaciju, AlphaCode će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `alphacode mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, AlphaCode sigurno čuva tokene u `~/.local/share/alphacode/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u AlphaCode, zajedno s ugrađenim alatima. Možete njima upravljati kroz AlphaCode konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete AlphaCode sa vašim Sentry nalogom.
