...
import type { HostPluginApi } from "@alphacode-ai/tui/plugin/slots"
...
export function createTuiPluginApi(opts: Opts = {}): HostPluginApi {
  ...
  const own = createAlphacodeClient({
  ...
  let selected = opts.theme?.selected ?? "alphacode"
  const set =
    opts.theme?.set ??
    ((name: string) => {
  ...
  return {
    ...
    attention: {
      ...
      soundboard: {
        ...
        current: () => opts.attention?.soundboard?.current?.() ?? "alphacode.default",
