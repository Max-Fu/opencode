...
エージェントスキルにより、AlphaCode はリポジトリまたはホームディレクトリから再利用可能な命令を検出できます。
...
AlphaCode は次の場所を検索します。

- プロジェクト設定: `.alphacode/skills/<name>/SKILL.md`
- グローバル設定: `~/.config/alphacode/skills/<name>/SKILL.md`
...
プロジェクトのローカルパスの場合、AlphaCode は現在の作業ディレクトリから git ワークツリーに到達するまで進みます。
途中で、一致する `skills/*/SKILL.md` を `.alphacode/` に読み込み、一致する `.claude/skills/*/SKILL.md` または `.agents/skills/*/SKILL.md` を読み込みます。

グローバル定義は、`~/.config/alphacode/skills/*/SKILL.md`、`~/.claude/skills/*/SKILL.md`、および `~/.agents/skills/*/SKILL.md` からもロードされます。
...
次のように `.alphacode/skills/git-release/SKILL.md` を作成します。
...
AlphaCode では、`skill` ツールの説明に利用可能なスキルがリストされています。
...
`alphacode.json` のパターンベースの権限を使用して、エージェントがアクセスできるスキルを制御します。
...
**組み込みエージェントの場合** (`alphacode.json` 内):
