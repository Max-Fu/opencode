...
U AlphaCode mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, AlphaCode koristi nasu `alphacode` temu.
...
AlphaCode dolazi sa vise ugradenih tema.
...
- Zele da AlphaCode odgovara izgledu njihovog terminala
...
AlphaCode podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/alphacode/themes/*.json` ili `$XDG_CONFIG_HOME/alphacode/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.alphacode/themes/*.json`
4. **Trenutni radni direktorij** - `./.alphacode/themes/*.json`
