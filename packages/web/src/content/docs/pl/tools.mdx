...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. AlphaCode posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w AlphaCode.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `ALPHACODE_EXPERIMENTAL_LSP_TOOL=true` (lub `ALPHACODE_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy AlphaCode lub gdy zmienna środowiskowa `ALPHACODE_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu AlphaCode:
