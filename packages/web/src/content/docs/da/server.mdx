---
title: Server
description: Interager med alphacode-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `alphacode serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en alphacode-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `alphacode.local` |
...
Indstil `ALPHACODE_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `alphacode`, eller indstil `ALPHACODE_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `alphacode serve` og `alphacode web`.
...
Når du kører `alphacode` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug alphacode-serveren til at interagere med alphacode programmatisk.
:::

Denne arkitektur lader alphacode understøtte flere klienter og lader dig interagere med alphacode programmatisk.

Du kan køre `alphacode serve` for at starte en selvstændig server. Hvis du har
alphacode TUI kørende, vil `alphacode serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af AlphaCode [IDE](/docs/ide) plugins.
...
AlphaCode-serveren viser følgende API'er.
