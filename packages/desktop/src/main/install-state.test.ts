...
describe("hasExistingAppState", () => {
  ...
  test("recognizes state written by an earlier AlphaCode launch", () => {
    expect(hasExistingAppState([file("alphacode.settings")])).toBe(true)
    expect(hasExistingAppState([file("alphacode.global.dat")])).toBe(true)
    expect(hasExistingAppState([file("window-state-abc.json")])).toBe(true)
    expect(hasExistingAppState([directory("alphacode")])).toBe(true)
  })
})
