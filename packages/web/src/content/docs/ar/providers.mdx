---
title: المزوّدون
description: استخدام أي موفّر LLM في AlphaCode.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم AlphaCode ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات AlphaCode.
...
في `~/.local/share/alphacode/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات AlphaCode.
...
AlphaCode Zen هي قائمة نماذج يوفّرها فريق AlphaCode وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع AlphaCode. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع AlphaCode Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر alphacode، ثم انتقل إلى [alphacode.ai/auth](https://alphacode.ai/auth).
...
يعمل مثل أي مزوّد آخر في AlphaCode واستخدامه اختياري بالكامل.

---

## AlphaCode Go

AlphaCode Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق AlphaCode، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع AlphaCode.

1. شغّل الأمر `/connect` في TUI، واختر `AlphaCode Go`، ثم انتقل إلى [alphacode.ai/auth](https://alphacode.ai/zen).
...
لاستخدام Amazon Bedrock مع AlphaCode:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل alphacode:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `alphacode.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل AlphaCode غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين alphacode لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل alphacode بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات alphacode.
...
تخزين الرمز في مساحة تخزين مصادقة alphacode.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم AlphaCode نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر AlphaCode
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`alphacode.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [alphacode-gitlab-auth](https://www.npmjs.com/package/alphacode-gitlab-auth).

##### التهيئة

خصّص عبر `alphacode.json`:
...
لاستخدام اشتراك GitHub Copilot مع alphacode:
...
لاستخدام Google Vertex AI مع AlphaCode:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر alphacode، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [alphacode-helicone-session](https://github.com/H2Shami/alphacode-helicone-session) لتسجيل كل محادثة AlphaCode تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة AlphaCode مدرجة كجلسة مستقلة.
...
يمكنك ضبط alphacode لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط alphacode لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط alphacode لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع AlphaCode. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/alphacode) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع AlphaCode:
...
3. انسخ مفتاح API لاستخدامه في AlphaCode.
...
6. **مهم**: قبل استخدام نماذج السحابة في AlphaCode، يجب سحب معلومات النموذج محليا:
...
AlphaCode Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق AlphaCode. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>AlphaCode Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **AlphaCode Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ AlphaCode.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات alphacode.
...
5. يمكنك أيضا تخصيصها عبر إعدادات alphacode. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع Alphacode:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات alphacode. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع alphacode. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `alphacode.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ AlphaCode بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `alphacode auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات alphacode وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات alphacode.
