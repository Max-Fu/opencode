---
title: LSP serveri
description: AlphaCode se integriše sa vašim LSP serverima.
---

AlphaCode se može integrisati sa Language Server Protocol (LSP) serverima kako bi koristio dijagnostiku kao feedback za agenta.

## Ugrađeni

AlphaCode dolazi sa nekoliko ugrađenih LSP servera za popularne jezike:
...
:::note
Možete onemogućiti automatska preuzimanja LSP servera tako što ćete postaviti varijablu okruženja `ALPHACODE_DISABLE_LSP_DOWNLOAD` na `true`.
:::
...
Kada je LSP omogućen i alphacode otvori fajl, on:
...
Možete omogućiti i prilagoditi LSP servere kroz `lsp` odjeljak u vašoj alphacode konfiguraciji.
