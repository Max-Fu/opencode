...
const AGENT_USERNAME = "alphacode-agent[bot]"
const AGENT_REACTION = "eyes"
const WORKFLOW_FILE = ".github/workflows/alphacode.yml"
  ...
  yield* Effect.promise(async () => {
    {
      ...
      function printNextSteps() {
        ...
        prompts.outro(
          [
            ...
            "",
            "   Learn more about the GitHub agent - https://alphacode.ai/docs/github/#usage-examples",
          ].join("\n"),
        )
      }
      ...
      async function installGitHubApp() {
        ...
        // Open browser
        const url = "https://github.com/apps/alphacode-agent"
        const command =
        ...
        async function getInstallation() {
          return await fetch(`https://api.alphacode.ai/get_github_app_installation?owner=${app.owner}&repo=${app.repo}`)
            .then((res) => res.json())
...
jobs:
  alphacode:
    if: |
      ...
      startsWith(github.event.comment.body, '/alphacode')
    ...
    steps:
      ...
      - name: Run alphacode
        uses: anomalyco/alphacode/github@latest${envStr}
        with:
...
export const githubRun = Effect.fn("Cli.github.run")(function* (args: { event?: string; token?: string }) {
  ...
  yield* Effect.promise(async () => {
    ...
    const shareBaseUrl = isMock ? "https://dev.alphacode.ai" : "https://alphacode.ai"
    ...
    function normalizeOidcBaseUrl(): string {
      ...
      if (!value) return "https://api.alphacode.ai"
    ...
    async function getUserPrompt() {
      ...
      const mentions = (process.env["MENTIONS"] || "/alphacode,/oc")
        .split(",")
    ...
    async function getOidcToken() {
      try {
        return await core.getIDToken("alphacode-github-action")
      } catch (error) {
    ...
    function generateBranchName(type: "issue" | "pr" | "schedule" | "dispatch") {
      ...
      if (type === "schedule" || type === "dispatch") {
        ...
        return `alphacode/${type}-${hex}-${timestamp}`
      }
      return `alphacode/${type}${issueId}-${timestamp}`
    }
  ...
  repository(owner: $owner, name: $repo) {
    ...
    function buildPromptDataForIssue(issue: GitHubIssue) {
      ...
      return [
        ...
        "- Git push and PR creation are handled AUTOMATICALLY by the alphacode infrastructure after your response",
