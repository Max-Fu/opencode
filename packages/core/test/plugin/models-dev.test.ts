...
import { ModelsDevPlugin } from "@alphacode-ai/core/plugin/models-dev"
...
describe("ModelsDevPlugin", () => {
  ...
  it.effect("registers key methods for providers with environment variables", () =>
    Effect.acquireUseRelease(
      Effect.sync(() => {
        const previous = {
          path: Flag.ALPHACODE_MODELS_PATH,
          disabled: Flag.ALPHACODE_DISABLE_MODELS_FETCH,
        }
        Flag.ALPHACODE_MODELS_PATH = path.join(import.meta.dir, "fixtures", "models-dev.json")
      ...
      (previous) =>
        Effect.sync(() => {
          Flag.ALPHACODE_MODELS_PATH = previous.path
          Flag.ALPHACODE_DISABLE_MODELS_FETCH = previous.disabled
        }),
    ),
  )
})
