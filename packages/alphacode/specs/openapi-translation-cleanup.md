...
Trim `packages/alphacode/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/alphacode/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/alphacode`.
- `[x]` `bun typecheck` from `packages/alphacode`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/alphacode/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/alphacode`.
- `[x]` `bun dev generate > /tmp/alphacode-openapi.json` from `packages/alphacode`.
...
- Edit `packages/alphacode/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/alphacode-openapi.json` from `packages/alphacode`.
...
- Check source schemas in files like `packages/alphacode/src/session/schema.ts`, `packages/alphacode/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/alphacode/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/alphacode/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
