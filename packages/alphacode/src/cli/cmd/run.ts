import type { PermissionV1 } from "@alphacode-ai/core/v1/permission"
import { FSUtil } from "@alphacode-ai/core/fs-util"
// CLI entry point for `alphacode run` and `alphacode --mini`.
//
// Handles three modes:
//   1. Non-interactive (default): sends a single prompt, streams events to
//      stdout, and exits when the session goes idle.
//   2. Interactive local (`alphacode --mini`): boots the split-footer direct mode
//      with an in-process server (no external HTTP).
//   3. Interactive attach (`alphacode --mini --attach`): connects to a running
//      alphacode server and runs interactive mode against it.
//
...
import { createAlphacodeClient, type AlphacodeClient, type ToolPart } from "@alphacode-ai/sdk/v2"
import { FormatError, FormatUnknownError } from "../error"
import { INTERACTIVE_INPUT_ERROR, resolveInteractiveStdin } from "./run/runtime.stdin"

type ModelInput = Parameters<AlphacodeClient["session"]["prompt"]>[0]["model"]
...
export const RunCommand = effectCmd({
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to ALPHACODE_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to ALPHACODE_SERVER_USERNAME or 'alphacode')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function session(sdk: AlphacodeClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: AlphacodeClient, sessionID: string) {
      ...
      async function current(sdk: AlphacodeClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: AlphacodeClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: AlphacodeClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: AlphacodeClient) {
        ...
        async function loop(client: AlphacodeClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
      ...
      const sdk = createAlphacodeClient({
        baseUrl: "http://alphacode.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)
