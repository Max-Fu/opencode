...
Du kan legge til eksterne verktøy i AlphaCode ved å bruke _Model Context Protocol_ eller MCP. AlphaCode støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [AlphaCode Config](https://alphacode.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="alphacode.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/alphacode`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
```jsonc title="alphacode.jsonc" {15}
{
  "$schema": "https://alphacode.ai/config.json",
  "mcp": {
    "my-local-mcp-server": {
...
AlphaCode håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil AlphaCode:
...
Hvis serveren krever autentisering, vil AlphaCode be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `alphacode mcp auth <server-name>`.
...
```json title="alphacode.json" {7-11}
{
  "$schema": "https://alphacode.ai/config.json",
  "mcp": {
    "my-oauth-server": {
...
```bash
alphacode mcp auth my-oauth-server
```
...
```bash
alphacode mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil AlphaCode lagre tokens sikkert i `~/.local/share/alphacode/mcp-auth.json`.
...
alphacode mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i AlphaCode, sammen med innebygde verktøy. Så du kan administrere dem gjennom AlphaCode-konfigurasjonen som alle andre verktøy.
...
```json title="alphacode.json" {11, 14-18}
{
  "$schema": "https://alphacode.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
alphacode mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble AlphaCode til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver AlphaCode-samtale oppført som en separat økt.
