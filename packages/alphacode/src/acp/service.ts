...
import type { AssistantMessage, Message, AlphacodeClient, SessionMessageResponse } from "@alphacode-ai/sdk/v2"
...
export const AuthMethodID = "alphacode-login"
...
export class Service extends Context.Service<Service, Interface>()("@alphacode/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `alphacode auth login` in the terminal",
      name: "Login with alphacode",
      id: AuthMethodID,
    }
...
function makeDirectoryService(sdk: AlphacodeClient) {
...
function makeUsageService(sdk: AlphacodeClient) {
...
async function loadDirectorySnapshot(sdk: AlphacodeClient, directory: string) {
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, alphacode provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const alphacodeProvider = providers[ProviderV2.ID.make("alphacode")]
  const alphacodeModel = alphacodeProvider ? Provider.sort(Object.values(alphacodeProvider.models))[0] : undefined
  if (alphacodeProvider && alphacodeModel) return { providerID: alphacodeProvider.id, modelID: alphacodeModel.id }
