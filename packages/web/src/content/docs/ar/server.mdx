---
title: الخادم
description: تفاعل مع خادم alphacode عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `alphacode serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل alphacode استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `alphacode.local` |
...
عيّن `ALPHACODE_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `alphacode`، أو عيّن `ALPHACODE_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `alphacode serve` و `alphacode web`.
...
عند تشغيل `alphacode` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم alphacode للتفاعل مع alphacode برمجيا.
:::

تتيح هذه البنية لـ alphacode دعم عدة عملاء وتمكّنك من التفاعل مع alphacode برمجيا.

يمكنك تشغيل `alphacode serve` لبدء خادم مستقل. إذا كانت واجهة alphacode في terminal (TUI)
قيد التشغيل، فسيبدأ `alphacode serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات AlphaCode لـ [IDE](/docs/ide).
...
يعرض خادم alphacode واجهات API التالية.
