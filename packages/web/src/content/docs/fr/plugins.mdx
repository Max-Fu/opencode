---
title: Plugins
description: Écrivez vos propres plugins pour étendre AlphaCode.
---

Les plugins vous permettent d'étendre AlphaCode en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de AlphaCode.
...
- `.alphacode/plugins/` - Plugins au niveau du projet
- `~/.config/alphacode/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/alphacode/node_modules/`.
...
1. Configuration globale (`~/.config/alphacode/alphacode.json`)
2. Configuration du projet (`alphacode.json`)
3. Répertoire global des plugins (`~/.config/alphacode/plugins/`)
4. Répertoire des plugins du projet (`.alphacode/plugins/`)
...
AlphaCode exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK alphacode pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre alphacode.
...
:::note
Si vous utilisez l'application de bureau AlphaCode, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez alphacode de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à alphacode :
...
L'assistant `tool` crée un outil personnalisé que alphacode peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour alphacode aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://alphacode.ai/docs/sdk) pour plus de détails.
