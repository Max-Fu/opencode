...
Con AlphaCode puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, AlphaCode usa nuestro propio tema `alphacode`.
...
AlphaCode viene con varios temas integrados.
...
- Quiere que AlphaCode coincida con la apariencia de su terminal
...
AlphaCode admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/alphacode/themes/*.json` o `$XDG_CONFIG_HOME/alphacode/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.alphacode/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.alphacode/themes/*.json`
