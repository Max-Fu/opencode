...
import { getAlphacodeModeStack, ALPHACODE_BASE_MODE, AlphacodeKeymapProvider, registerAlphacodeKeymap } from "../src/keymap"
...
test("legacy page key aliases compile as page keys", async () => {
  ...
  function Harness() {
    ...
    const offKeymap = registerAlphacodeKeymap(keymap, renderer, config)
...
test("mode-less bindings stay active when alphacode mode changes", async () => {
  ...
  function Harness() {
    ...
    const popQuestion = getAlphacodeModeStack(keymap).push("question")
    counts.question = activeCounts()
    popQuestion()
    const popAutocomplete = getAlphacodeModeStack(keymap).push("autocomplete")
