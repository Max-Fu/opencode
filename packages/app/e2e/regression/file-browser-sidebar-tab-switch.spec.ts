...
const directory = "C:/AlphaCode/FileBrowserSidebar"
...
async function setup(page: Page) {
  await mockAlphaCodeServer(page, {
    ...
    provider: {
      ...
      connected: ["alphacode"],
      default: { providerID: "alphacode", modelID: "test" },
    },
    sessions: [
      {
        id: sessionID,
  ...
  await page.addInitScript(
    ({ directory, server, sessionID }) => {
      ...
      localStorage.setItem(
        "alphacode.global.dat:layout",
        JSON.stringify({ review: { diffStyle: "split", panelOpened: true } }),
      )
      localStorage.setItem(
        "alphacode.global.dat:review-panel-v2",
        JSON.stringify({ sidebarOpened: true, sidebarWidth: 240, expandMode: "collapse" }),
      )
      localStorage.setItem(
        "alphacode.window.browser.dat:tabs",
