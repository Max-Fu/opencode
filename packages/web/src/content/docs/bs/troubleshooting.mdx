...
Da biste otklonili probleme s AlphaCode, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\alphacode\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `alphacode --log-level DEBUG`.

---

## Pohrana

AlphaCode pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/alphacode/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\alphacode`
...
AlphaCode Desktop pokreće lokalni AlphaCode server (`alphacode-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `AlphaCode` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/alphacode/alphacode.jsonc` (ili `~/.config/alphacode/alphacode.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/alphacode/alphacode.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\alphacode\alphacode.jsonc`
...
AlphaCode također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/alphacode/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\alphacode\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga AlphaCode mogao ponovo izgraditi.

1. Potpuno zatvorite AlphaCode Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/alphacode`
- **Linux**: obrišite `~/.cache/alphacode` (ili pokrenite `rm -rf ~/.cache/alphacode`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\alphacode`

3. Ponovo pokrenite AlphaCode Desktop.

---

### Rješavanje problema sa vezom na serveru

AlphaCode Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `alphacode.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `ALPHACODE_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `ALPHACODE_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, AlphaCode Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa AlphaCode karakteristikama.

---

### Obavještenja se ne prikazuju

AlphaCode Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za AlphaCode u postavkama vašeg OS-a, i
...
1. Zatvorite AlphaCode Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka AlphaCode Desktop aplikacije):

- `alphacode.settings.dat` (URL zadanog servera za desktop)
- `alphacode.global.dat` i `alphacode.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Ako imate problema s AlphaCode:
...
Da saznate kojim modelima imate pristup, pokrenite `alphacode models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\alphacode`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. AlphaCode dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\alphacode`

2. Ponovo pokrenite AlphaCode da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti AlphaCode da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
AlphaCode će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
