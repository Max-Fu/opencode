...
Umiejętności agenta pozwalają alphacode odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
alphacode przeszukuje te lokalizacje:

- Project config: `.alphacode/skills/<name>/SKILL.md`
- Global config: `~/.config/alphacode/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu alphacode przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.alphacode/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/alphacode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.alphacode/skills/git-release/SKILL.md` w ten sposób:
...
alphacode wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `alphacode.json`:
...
**Dla agentów wbudowanych** (w `alphacode.json`):
