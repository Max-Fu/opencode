---
title: Server
description: Nutze den alphacode-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `alphacode serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein alphacode-Client nutzen kann.
...
Setze `ALPHACODE_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `alphacode`, kann aber mit `ALPHACODE_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `alphacode serve` und `alphacode web`.
...
Wenn du `alphacode` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den alphacode-Server, um alphacode programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `alphacode serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `alphacode serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die AlphaCode-[IDE](/docs/ide)-Plugins.
...
Der alphacode-Server stellt folgende APIs bereit.
