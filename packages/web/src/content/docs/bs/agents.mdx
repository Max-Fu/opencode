...
Postoje dvije vrste agenata u AlphaCode: primarni agenti i podagenti.
...
AlphaCode dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
AlphaCode dolazi sa tri ugrađena subagenta, **General**, **Explore** i **Scout**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

AlphaCode dolazi sa dva ugrađena primarna agenta i tri ugrađena subagenta.
...
Agent samo za čitanje za istraživanje eksterne dokumentacije i zavisnosti. Koristite ga kada trebate klonirati repozitorij zavisnosti u AlphaCode-ov upravljani cache, pregledati izvorni kod biblioteke ili uporediti lokalni kod sa upstream implementacijama bez mijenjanja vašeg radnog prostora.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `alphacode.json`:
...
- Globalno: `~/.config/alphacode/agents/`
- Po projektu: `.alphacode/agents/`

```markdown title="~/.config/alphacode/agents/review.md"
---
...
Ako temperatura nije navedena, AlphaCode koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu AlphaCode konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj AlphaCode konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [AlphaCode Zen](/docs/zen), koristili biste `alphacode/gpt-5.1-codex` za GPT 5.1 Codex.
...
```json title="alphacode.json" {3-6,9-12}
{
  "$schema": "https://alphacode.ai/config.json",
  "tools": {
    "write": true,
...
```json title="alphacode.json" {3-5,8-10}
{
  "$schema": "https://alphacode.ai/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Pokrenite `alphacode models` da vidite listu dostupnih modela.
:::
...
:::tip
Imate li agenta kojeg biste željeli podijeliti? [Pošalji PR](https://github.com/anomalyco/alphacode).
:::
...
```markdown title="~/.config/alphacode/agents/docs-writer.md"
---
...
```markdown title="~/.config/alphacode/agents/security-auditor.md"
---
