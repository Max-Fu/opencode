...
import { createAlphacodeClient } from "@alphacode-ai/sdk/v2/client"
import { ServerAuth } from "@alphacode-ai/server/auth"
...
export interface Interface {
  readonly client: () => Effect.Effect<ReturnType<typeof createAlphacodeClient>, unknown>
...
export class Service extends Context.Service<Service, Interface>()("@alphacode/cli/Daemon") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const createClient = Effect.fnUntraced(function* (url: string) {
      return createAlphacodeClient({ baseUrl: url, headers: ServerAuth.headers({ password: yield* password() }) })
    })
    ...
    const client = Effect.fn("cli.daemon.client")(function* () {
      ...
      return createAlphacodeClient({ baseUrl: connection.url, headers: connection.headers })
    })
