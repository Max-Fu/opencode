...
Per diagnosticare problemi con AlphaCode, inizia controllando i log e i dati locali che salva su disco.
...
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\alphacode\log`

I file di log hanno nomi con timestamp (ad es. `2025-01-09T123456.log`) e vengono conservati i 10 file di log piu recenti.

Puoi impostare il livello di log con l'opzione a riga di comando `--log-level` per ottenere informazioni di debug piu dettagliate. Per esempio: `alphacode --log-level DEBUG`.

---

## Archiviazione

alphacode salva i dati delle sessioni e altri dati dell'applicazione su disco in:

- **macOS/Linux**: `~/.local/share/alphacode/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\alphacode`
...
AlphaCode Desktop esegue in background un server locale di AlphaCode (il sidecar `alphacode-cli`). La maggior parte dei problemi e causata da un plugin che si comporta male, da una cache corrotta o da un'impostazione del server errata.
...
- Solo macOS: menu `AlphaCode` -> **Reload Webview** (utile se l'interfaccia e vuota o bloccata).
...
- **macOS/Linux** (installazioni vecchie): `~/.local/share/alphacode/alphacode.jsonc`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\alphacode\alphacode.jsonc`
...
AlphaCode puo anche caricare plugin locali dal disco. Spostali temporaneamente altrove (o rinomina la cartella) e riavvia l'app desktop:

- **Plugin globali**
  - **macOS/Linux**: `~/.config/alphacode/plugins/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\alphacode\plugins`
...
Se disabilitare i plugin non aiuta (o l'installazione di un plugin e bloccata), svuota la cache in modo che AlphaCode possa ricostruirla.

1. Chiudi completamente AlphaCode Desktop.
2. Elimina la directory della cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/alphacode`
- **Linux**: elimina `~/.cache/alphacode` (oppure esegui `rm -rf ~/.cache/alphacode`)
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.cache\alphacode`

3. Riavvia AlphaCode Desktop.

---

### Risolvi problemi di connessione al server

AlphaCode Desktop puo avviare il proprio server locale (predefinito) oppure connettersi a un URL server che hai configurato.
...
Se il tuo `alphacode.json(c)` contiene una sezione `server`, rimuovila temporaneamente e riavvia l'app desktop.

#### Controlla le variabili d'ambiente

Se hai `ALPHACODE_PORT` impostato nell'ambiente, l'app desktop provera a usare quella porta per il server locale.

- Rimuovi `ALPHACODE_PORT` (o scegli una porta libera) e riavvia.
...
Su Windows, AlphaCode Desktop richiede **WebView2 Runtime** di Microsoft Edge. Se l'app si apre su una finestra vuota o non parte, installa/aggiorna WebView2 e riprova.

---

### Windows: problemi generali di prestazioni

Se riscontri prestazioni lente, problemi di accesso ai file o problemi del terminale su Windows, prova a usare [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL fornisce un ambiente Linux che funziona in modo piu fluido con le funzionalita di AlphaCode.

---

### Notifiche non visualizzate

AlphaCode Desktop mostra le notifiche di sistema solo quando:

- le notifiche sono abilitate per AlphaCode nelle impostazioni del sistema operativo, e
...
1. Chiudi AlphaCode Desktop.
2. Trova ed elimina questi file (si trovano nella directory dati dell'app AlphaCode Desktop):

- `alphacode.settings.dat` (desktop default server URL)
- `alphacode.global.dat` e `alphacode.workspace.*.dat` (stato dell'interfaccia come server/progetti recenti)
...
Se riscontri problemi con AlphaCode:
...
Per capire a quali modelli hai accesso, esegui `alphacode models`
...
2. Se il problema persiste, prova a cancellare la configurazione salvata:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.local\share\alphacode`
...
Se incontri errori nelle chiamate API, potrebbe dipendere da pacchetti provider non aggiornati. alphacode installa dinamicamente i pacchetti provider (OpenAI, Anthropic, Google, ecc.) quando servono e li mette in cache localmente.
...
1. Svuota la cache dei pacchetti provider:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.cache\alphacode`

2. Riavvia alphacode per reinstallare i pacchetti provider piu recenti

Questo forzera alphacode a scaricare le versioni piu recenti dei pacchetti provider, cosa che spesso risolve problemi di compatibilita con parametri dei modelli e cambiamenti delle API.
...
alphacode rilevera se stai usando Wayland e preferira `wl-clipboard`; altrimenti provera a trovare gli strumenti per gli appunti nell'ordine: `xclip` e `xsel`.
