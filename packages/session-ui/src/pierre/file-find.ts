...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("alphacode-find")
  api.delete("alphacode-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("alphacode-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("alphacode-find", new Highlight(...rest))
    return true
  }
