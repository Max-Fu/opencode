---
title: Plugin
description: Scrivi plugin per estendere AlphaCode.
---

I plugin ti permettono di estendere AlphaCode agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di AlphaCode.
...
- `.alphacode/plugins/` - plugin a livello progetto
- `~/.config/alphacode/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/alphacode/node_modules/`.
...
1. Config globale (`~/.config/alphacode/alphacode.json`)
2. Config di progetto (`alphacode.json`)
3. Directory plugin globale (`~/.config/alphacode/plugins/`)
4. Directory plugin di progetto (`.alphacode/plugins/`)
...
AlphaCode esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di alphacode per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere alphacode.
...
:::note
Se usi l'app desktop di AlphaCode, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a alphacode di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a alphacode:
...
L'helper `tool` crea uno strumento personalizzato che alphacode può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in alphacode insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://alphacode.ai/docs/sdk) per i dettagli.
