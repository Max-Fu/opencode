...
Mit AlphaCode kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet AlphaCode das `alphacode`-Theme.
...
AlphaCode liefert mehrere eingebaute Themes mit.
...
- AlphaCode optisch ans Terminal anpassen wollen
...
AlphaCode unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/alphacode/themes/*.json` oder `$XDG_CONFIG_HOME/alphacode/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.alphacode/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.alphacode/themes/*.json`
