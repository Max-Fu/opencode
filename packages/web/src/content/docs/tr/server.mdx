---
title: Sunucu
description: alphacode sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`alphacode serve` komutu, alphacode istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `alphacode.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `ALPHACODE_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `alphacode` değeridir; değiştirmek isterseniz `ALPHACODE_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `alphacode serve` hem de `alphacode web` için geçerlidir.
...
`alphacode` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
alphacode ile programatik etkileşim için alphacode sunucusunu kullanın.
:::

Bu mimari, alphacode'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`alphacode serve` ile bağımsız bir sunucu başlatabilirsiniz. alphacode TUI açıksa, `alphacode serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum alphacode [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
alphacode sunucusu aşağıdaki API'leri sunar.
