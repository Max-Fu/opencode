...
import { SessionStatusEvent } from "@alphacode-ai/schema/session-status-event"
import { SessionV1 } from "@alphacode-ai/schema/session-v1"
import type {
  AssistantMessage,
  GlobalEvent,
  Message,
...
export const directory = "C:/AlphaCode/TimelineStability"
...
export const model = { providerID: "alphacode", modelID: "claude-opus-4-6", variant: "max" }
...
export async function setupTimeline(
  ...
  if (input.locale) {
    await page.addInitScript((locale) => {
      localStorage.setItem("alphacode.global.dat:language", JSON.stringify({ locale }))
    }, input.locale)
  }
...
function provider() {
  return {
    ...
    connected: ["alphacode"],
    default: { providerID: "alphacode", modelID: "claude-opus-4-6" },
  }
}
