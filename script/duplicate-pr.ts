...
import { createAlphacode } from "@alphacode-ai/sdk"
...
`)
  ...
  const alphacode = await createAlphacode({ port: 0 })
  ...
  try {
    ...
    const session = await alphacode.client.session.create()
    const result = await alphacode.client.session
      .prompt({
