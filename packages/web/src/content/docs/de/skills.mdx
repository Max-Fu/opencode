...
Agent Skills erlauben AlphaCode, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
AlphaCode sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft AlphaCode vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.alphacode/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/alphacode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.alphacode/skills/git-release/SKILL.md` zum Beispiel so:
...
AlphaCode listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `alphacode.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `alphacode.json`):
