# Remove `packages/alphacode/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/alphacode/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the alphacode legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/alphacode/src/storage/db.ts` are concentrated in 22 source files:

- `packages/alphacode/src/account/repo.ts`
- `packages/alphacode/src/cli/cmd/db.ts`
- `packages/alphacode/src/cli/cmd/import.ts`
- `packages/alphacode/src/cli/cmd/stats.ts`
- `packages/alphacode/src/control-plane/workspace.ts`
- `packages/alphacode/src/index.ts`
- `packages/alphacode/src/node.ts`
- `packages/alphacode/src/permission/index.ts`
- `packages/alphacode/src/project/project.ts`
- `packages/alphacode/src/server/projectors.ts`
- `packages/alphacode/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/alphacode/src/server/shared/fence.ts`
- `packages/alphacode/src/session/message-v2.ts`
- `packages/alphacode/src/session/projectors.ts`
- `packages/alphacode/src/session/prompt.ts`
- `packages/alphacode/src/session/session.ts`
- `packages/alphacode/src/session/todo.ts`
- `packages/alphacode/src/share/share-next.ts`
- `packages/alphacode/src/storage/db.ts`
- `packages/alphacode/src/sync/index.ts`
- `packages/alphacode/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy alphacode database wrapper; `packages/alphacode/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the alphacode wrapper.
...
Status: Completed. `SyncEvent` and the alphacode projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy alphacode database wrapper.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy alphacode database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy alphacode database wrapper.
...
All migration groups are complete or superseded. `packages/alphacode/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No alphacode data-migration group remains.

The previous alphacode `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate alphacode data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/alphacode`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/alphacode/src`
- `bun typecheck` from `packages/alphacode`
- Relevant package tests from `packages/alphacode`, not the repo root
