...
// terminal sizes. The default alphacode tui command is excluded —
// `alphacode --help` includes an ASCII banner that pulls in the install
...
// Top-level commands. Order matches what `alphacode --help` prints today;
...
// top-level help on `--help` and exits 1; not a real alphacode command.
const TOP_LEVEL = [
  "acp",
  "mcp",
  "attach",
...
describe("alphacode CLI help-text snapshots", () => {
  ...
  cliIt.live(
    ...
    ({ alphacode }) =>
      Effect.gen(function* () {
        const topLevel = yield* alphacode.spawn(["--help"], { env: SNAPSHOT_ENV })
        ...
        const [failures, results] = yield* Effect.partition(
          ...
          (argv) =>
            Effect.gen(function* () {
              const result = yield* alphacode.spawn([...argv, "--help"], { env: SNAPSHOT_ENV })
              if (result.exitCode !== 0) {
                return yield* Effect.fail(`alphacode ${argv.join(" ")}: exit ${result.exitCode}`)
              }
              return { argv, result }
            }),
          { concurrency: 8 },
        ...
        for (const { argv, result } of results) {
          ...
          expect(normalize(result.stderr)).toMatchSnapshot(`alphacode ${argv.join(" ")} --help`)
        }
