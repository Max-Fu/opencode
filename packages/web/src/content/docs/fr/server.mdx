---
title: Serveur
description: Interagissez avec le serveur alphacode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `alphacode serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client alphacode peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `alphacode.local` |
...
Définissez `ALPHACODE_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `alphacode`, ou définissez `ALPHACODE_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `alphacode serve` et à `alphacode web`.
...
Lorsque vous exécutez `alphacode`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur alphacode pour interagir avec alphacode par programmation.
:::

Cette architecture permet à alphacode de prendre en charge plusieurs clients et vous permet d'interagir avec alphacode par programme.

Vous pouvez exécuter `alphacode serve` pour démarrer un serveur autonome. Si vous avez le
alphacode TUI en cours d'exécution, `alphacode serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins AlphaCode [IDE](/docs/ide).
...
Le serveur alphacode expose les API suivantes.
