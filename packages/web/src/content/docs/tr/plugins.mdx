---
title: Eklentiler
description: AlphaCode'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek AlphaCode'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya AlphaCode'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.alphacode/plugins/` - Proje düzeyinde eklentiler
- `~/.config/alphacode/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/alphacode/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/alphacode/alphacode.json`)
2. Project config (`alphacode.json`)
3. Global eklenti dizini (`~/.config/alphacode/plugins/`)
4. Proje eklenti dizini (`.alphacode/plugins/`)
...
AlphaCode bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir AlphaCode SDK istemcisi.
...
AlphaCode'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
AlphaCode masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
AlphaCode'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca AlphaCode'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, AlphaCode'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra AlphaCode'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://alphacode.ai/docs/sdk) bakın.
