...
import type { Event, AlphacodeClient } from "@alphacode-ai/sdk/v2"
...
type PermissionReplyParams = Parameters<AlphacodeClient["permission"]["reply"]>[0]
...
async function tempFile(name: string, content: string) {
  const dir = await mkdtemp(path.join(tmpdir(), "alphacode-acp-permission-"))
