...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.alphacode.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `alphacode.settings.dat` is special: it maps to the `alphacode.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"alphacode.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // alphacode.settings.dat → the electron settings store ("alphacode.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "alphacode.global.dat").
  const storeName = filename === "alphacode.settings.dat" ? "alphacode.settings" : filename
