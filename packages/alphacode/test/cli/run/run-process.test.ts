// Subprocess integration tests for `alphacode run` (non-interactive mode).
// These exercise the real CLI binary against a TestLLMServer running in the
// same process. See `test/lib/cli-process.ts` for the harness — each test uses
// `alphacode.run(message, opts?)` to spawn `bun src/index.ts run ...` with
// `ALPHACODE_CONFIG_CONTENT` providing the test provider config inline.
import { describe, expect } from "bun:test"
import { Effect } from "effect"
import { reply } from "../../lib/llm-server"
import { cliIt } from "../../lib/cli-process"

describe("alphacode run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, alphacode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* alphacode.run("say hi")
        alphacode.expectExit(result, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, alphacode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* alphacode.run("use a tool", {
  ...
  cliIt.concurrent(
    ...
    ({ llm, alphacode }) =>
      Effect.gen(function* () {
        ...
        const thinking = yield* alphacode.run("think", { extraArgs: ["--thinking"] })
        alphacode.expectExit(thinking, 0)
        expect(thinking.stdout).toBe("Thinking: considering\nanswer\n")

        yield* llm.reason("hidden", { text: "visible" })
        const plain = yield* alphacode.run("think again")
        alphacode.expectExit(plain, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, alphacode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* alphacode.run("trigger midstream error", { timeoutMs: 30_000 })
  ...
  cliIt.concurrent(
    ...
    ({ llm, alphacode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* alphacode.run("say hi", { format: "json" })
        alphacode.expectExit(result, 0)

        const events = alphacode.parseJsonEvents(result.stdout)
  ...
  cliIt.concurrent(
    ...
    ({ alphacode }) =>
      Effect.gen(function* () {
        const result = yield* alphacode.run("use an unknown model", {
          model: "test/nonexistent-model",
          format: "json",
        })
  ...
  cliIt.concurrent(
    ...
    ({ llm, alphacode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* alphacode.run("exercise json records", {
          format: "json",
  ...
  cliIt.concurrent(
    ...
    ({ llm, alphacode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* alphacode.run("fail after output", { format: "json" })
  ...
  cliIt.concurrent(
    ...
    ({ home, llm, alphacode }) =>
      Effect.gen(function* () {
        ...
        const denied = yield* alphacode.run("request permission", { permission: { bash: "ask" } })
        alphacode.expectExit(denied, 0)
        ...
        const allowed = yield* alphacode.run("request permission", {
          permission: { bash: "ask" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        alphacode.expectExit(allowed, 0)
        ...
        const explicitlyDenied = yield* alphacode.run("request denied permission", {
          permission: { bash: "deny" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        alphacode.expectExit(explicitlyDenied, 0)
  ...
  cliIt.live(
    ...
    ({ home, llm, alphacode }) =>
      Effect.gen(function* () {
        ...
        const server = yield* alphacode.serve()

        const result = yield* alphacode.run("read the attachment", {
  ...
  cliIt.concurrent(
    ...
    ({ home, alphacode }) =>
      Effect.gen(function* () {
        const result = yield* alphacode.run("read the directory", {
  ...
  cliIt.live(
    ...
    ({ llm, alphacode }) =>
      Effect.gen(function* () {
        yield* llm.hang
        const run = yield* alphacode.startRun("wait forever")
        yield* llm.wait(1)
        run.interrupt()
