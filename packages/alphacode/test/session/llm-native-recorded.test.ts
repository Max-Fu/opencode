...
import { SessionV1 } from "@alphacode-ai/core/v1/session"
import { ModelsDev } from "@alphacode-ai/core/models-dev"
import { HttpRecorder } from "@alphacode-ai/http-recorder"
import { HttpRecorderInternal } from "@alphacode-ai/http-recorder/internal"
...
import { LLMEvent, LLMResponse } from "@alphacode-ai/llm"
import { RequestExecutor } from "@alphacode-ai/llm/route"
...
const zenURL = (connection: string) => `https://console.alphacode.ai/proxy/connections/${connection}/v1`
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.ALPHACODE_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["alphacode", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("ALPHACODE_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("ALPHACODE_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["alphacode", "native", "oauth", "tool-loop"],
  ...
  {
    id: "alphacode-proxy",
    name: "AlphaCode proxy",
    providerID: ProviderV2.ID.alphacode,
    modelID: "gpt-5.2-codex",
    cassette: "session/native-zen-tool-loop",
    protocol: "openai-responses",
    tags: ["alphacode", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.ALPHACODE_RECORD_CONSOLE_TOKEN && process.env.ALPHACODE_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.alphacode,
        name: "AlphaCode Zen",
        env: ["ALPHACODE_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: zenURL(process.env.ALPHACODE_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.ALPHACODE_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.ALPHACODE_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["alphacode", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("ALPHACODE_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("ALPHACODE_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("ALPHACODE_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set ALPHACODE_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the alphacode LLM stack remain real.
  const metadata = {
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      path.join(directory, "alphacode.json"),
      JSON.stringify({ $schema: "https://alphacode.ai/config.json", ...scenario.config(model) }),
    ),
  )
