---
title: Config
description: Using the AlphaCode JSON config.
---

You can configure AlphaCode using a JSON config file.

---

## Format

AlphaCode supports both **JSON** and **JSONC** (JSON with Comments) formats.
...
1. **Remote config** (from `.well-known/alphacode`) - organizational defaults
2. **Global config** (`~/.config/alphacode/alphacode.json`) - user preferences
3. **Custom config** (`ALPHACODE_CONFIG` env var) - custom overrides
4. **Project config** (`alphacode.json` in project) - project-specific settings
5. **`.alphacode` directories** - agents, commands, plugins
6. **Inline config** (`ALPHACODE_CONFIG_CONTENT` env var) - runtime overrides
7. **Managed config files** (`/Library/Application Support/alphacode/` on macOS) - admin-controlled
...
:::note
The `.alphacode` and `~/.config/alphacode` directories use **plural names** for subdirectories: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, and `themes/`. Singular names (e.g., `agent/`) are also supported for backwards compatibility.
:::
...
Organizations can provide default configuration via the `.well-known/alphacode` endpoint. This is fetched automatically when you authenticate with a provider that supports it.
...
```json title="Remote config from .well-known/alphacode"
{
  "mcp": {
    "jira": {
      "type": "remote",
...
Place your global AlphaCode config in `~/.config/alphacode/alphacode.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.

For TUI-specific settings, use `~/.config/alphacode/tui.json`.
...
Add `alphacode.json` in your project root. Project config has the highest precedence among standard config files - it overrides both global and remote configs.
...
When AlphaCode starts up, it first looks for a config file in the current directory, then traverses up to the nearest Git directory.
...
Specify a custom config file path using the `ALPHACODE_CONFIG` environment variable.

```bash
export ALPHACODE_CONFIG=/path/to/my/custom-config.json
alphacode run "Hello world"
```
...
Specify a custom config directory using the `ALPHACODE_CONFIG_DIR`
environment variable. This directory will be searched for agents, commands,
modes, and plugins just like the standard `.alphacode` directory, and should
follow the same structure.

```bash
export ALPHACODE_CONFIG_DIR=/path/to/my/config-directory
alphacode run "Hello world"
```

The custom directory is loaded after the global config and `.alphacode` directories, so it **can override** their settings.
...
Drop an `alphacode.json` or `alphacode.jsonc` file in the system managed config directory:

| Platform | Path                                     |
| -------- | ---------------------------------------- |
| macOS    | `/Library/Application Support/alphacode/` |
| Linux    | `/etc/alphacode/`                         |
| Windows  | `%ProgramData%\alphacode`                 |

These directories require admin/root access to write, so users cannot modify them.

#### macOS managed preferences

On macOS, AlphaCode reads managed preferences from the `ai.alphacode.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.

AlphaCode checks these paths:

1. `/Library/Managed Preferences/<user>/ai.alphacode.managed.plist`
2. `/Library/Managed Preferences/ai.alphacode.managed.plist`

The plist keys map directly to `alphacode.json` fields. MDM metadata keys (`PayloadUUID`, `PayloadType`, etc.) are stripped automatically.

**Creating a `.mobileconfig`**

Use the `ai.alphacode.managed` PayloadType. The AlphaCode config keys go directly in the payload dict:
...
<dict>
  ...
  <array>
    <dict>
      <key>PayloadType</key>
      <string>ai.alphacode.managed</string>
      <key>PayloadIdentifier</key>
      <string>com.example.alphacode.config</string>
      <key>PayloadUUID</key>
  ...
  <key>PayloadIdentifier</key>
  <string>com.example.alphacode</string>
  <key>PayloadUUID</key>
  <string>GENERATE-YOUR-OWN-UUID</string>
  <key>PayloadVersion</key>
  <integer>1</integer>
...
The server/runtime config schema is defined in [**`alphacode.ai/config.json`**](https://alphacode.ai/config.json).

TUI config uses [**`alphacode.ai/tui.json`**](https://alphacode.ai/tui.json).
...
Use `ALPHACODE_TUI_CONFIG` to point to a custom TUI config file.
...
Legacy `theme`, `keybinds`, and `tui` keys in `alphacode.json` are deprecated and automatically migrated when possible.

---

### Server

You can configure server settings for the `alphacode serve` and `alphacode web` commands through the `server` option.
...
- `mdns` - Enable mDNS service discovery. This allows other devices on the network to discover your AlphaCode server.
- `mdnsDomain` - Custom domain name for mDNS service. Defaults to `alphacode.local`. Useful for running multiple instances on the same network.
...
If not specified, AlphaCode will automatically discover and use a sensible default based on your operating system (e.g. `pwsh` or `cmd.exe` on Windows, `/bin/zsh` or `/bin/bash` on macOS/Linux). You can provide an absolute path or a short name.
...
You can configure the providers and models you want to use in your AlphaCode config through the `provider`, `model` and `small_model` options.
...
The `small_model` option configures a separate model for lightweight tasks like title generation. By default, AlphaCode tries to use a cheaper model if one is available from your provider, otherwise it falls back to your main model.
...
Use the `experimental.policies` option to allow or deny AlphaCode actions on configured resources. Currently, policies can control which providers AlphaCode may use.
...
AlphaCode normalizes image attachments before sending them to the model. By default, images are resized when they exceed `2000x2000` pixels or `5242880` base64 bytes.
...
If an image still cannot fit after resizing, AlphaCode omits oversized tool-result images or fails oversized user-provided images with an image size error.
...
You can also define agents using markdown files in `~/.config/alphacode/agents/` or `.alphacode/agents/`. [Learn more here](/docs/agents).
...
The default agent must be a primary agent (not a subagent). This can be a built-in agent like `"build"` or `"plan"`, or a [custom agent](/docs/agents) you've defined. If the specified agent doesn't exist or is a subagent, AlphaCode will fall back to `"build"` with a warning.

This setting applies across all interfaces: TUI, CLI (`alphacode run`), desktop app, and GitHub Action.
...
You can also define commands using markdown files in `~/.config/alphacode/commands/` or `.alphacode/commands/`. [Learn more here](/docs/commands).
...
AlphaCode uses snapshots to track file changes during agent operations, enabling you to undo and revert changes within a session. Snapshots are enabled by default.
...
AlphaCode will automatically download any new updates when it starts up. You can disable this with the `autoupdate` option.
...
By default, alphacode **allows all operations** without requiring explicit approval. You can change this using the `permission` option.
...
[Plugins](/docs/plugins) extend AlphaCode with custom tools, hooks, and integrations.

Place plugin files in `.alphacode/plugins/` or `~/.config/alphacode/plugins/`. You can also load plugins from npm through the `plugin` option.

```json title="alphacode.json"
{
  "$schema": "https://alphacode.ai/config.json",
  "plugin": ["alphacode-helicone-session", "@my-org/custom-plugin"]
}
```
...
This is useful when you want to restrict AlphaCode to only use specific providers rather than disabling them one by one.
