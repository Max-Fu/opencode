...
Dzięki alphacode możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie alphacode używa naszego własnego motywu `alphacode`.
...
alphacode ma kilka wbudowanych motywów.
...
- Chcą, aby AlphaCode pasował do wyglądu ich terminala
...
alphacode obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/alphacode/themes/*.json` lub `$XDG_CONFIG_HOME/alphacode/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.alphacode/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.alphacode/themes/*.json`
