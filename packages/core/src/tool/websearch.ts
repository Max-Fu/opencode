...
import { ToolFailure } from "@alphacode-ai/llm"
...
export class ConfigService extends Context.Service<ConfigService, Config>()("@alphacode/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.ALPHACODE_WEBSEARCH_PROVIDER === "exa" || process.env.ALPHACODE_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.ALPHACODE_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("ALPHACODE_EXPERIMENTAL") || truthy("ALPHACODE_ENABLE_EXA") || truthy("ALPHACODE_EXPERIMENTAL_EXA"),
    enableParallel: truthy("ALPHACODE_ENABLE_PARALLEL") || truthy("ALPHACODE_EXPERIMENTAL_PARALLEL"),
              ...
              const text =
                provider === "exa"
                  ...
                  : yield* callMcp(
                      ...
                      {
                        "User-Agent": `alphacode/${InstallationVersion}`,
