...
import { Watcher } from "@alphacode-ai/core/filesystem/watcher"
...
const flagsLayer = ConfigProvider.layer(
  ConfigProvider.fromUnknown({
    ALPHACODE_EXPERIMENTAL_FILEWATCHER: "true",
    ALPHACODE_EXPERIMENTAL_DISABLE_FILEWATCHER: "false",
  }),
)
...
function withTmp<A, E, R>(
  ...
  return Effect.acquireRelease(
    Effect.promise(async () => {
      ...
      await $`git config user.email test@alphacode.test`.cwd(tmp.path).quiet()
