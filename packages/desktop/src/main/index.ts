...
const TEST_ONBOARDING = process.env.ALPHACODE_TEST_ONBOARDING === "1"
const SIDECAR_VERSION = process.env.ALPHACODE_SIDECAR_V2 === "1" ? "v2" : "v1"
...
const main = Effect.gen(function* () {
  ...
  process.env.ALPHACODE_DISABLE_EMBEDDED_WEB_UI = "true"

  const appId = app.isPackaged ? APP_IDS[CHANNEL] : "ai.alphacode.desktop.dev"
  const onboardingTestRoot = ((): string | undefined => {
    if (!TEST_ONBOARDING) return

    const root = join(tmpdir(), `alphacode-onboarding-${randomUUID()}`)
  ...
  app.on("second-instance", (_event: Event, argv: string[]) => {
    const urls = argv.filter((arg: string) => arg.startsWith("alphacode://"))
    if (urls.length) {
  ...
  app.setAsDefaultProtocolClient("alphacode")
  registerRendererProtocol()
  setDockIcon()
  ...
  const loadingTask = yield* Effect.gen(function* () {
    ...
    const port = yield* Effect.gen(function* () {
      const fromEnv = process.env.ALPHACODE_PORT
      if (fromEnv) {
