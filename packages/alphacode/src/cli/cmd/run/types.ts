// Shared type vocabulary for the direct interactive mode (`alphacode --mini`).
//
...
import type { AlphacodeClient, PermissionRequest, QuestionRequest, ToolPart } from "@alphacode-ai/sdk/v2"
import type { TuiConfig } from "@alphacode-ai/tui/config"
...
type PromptModel = Parameters<AlphacodeClient["session"]["prompt"]>[0]["model"]
type PromptInput = Parameters<AlphacodeClient["session"]["prompt"]>[0]

export type RunPromptPart = NonNullable<PromptInput["parts"]>[number]

export type RunCommand = NonNullable<Awaited<ReturnType<AlphacodeClient["command"]["list"]>>["data"]>[number]

export type RunProvider = NonNullable<Awaited<ReturnType<AlphacodeClient["provider"]["list"]>>["data"]>["all"][number]
...
export type RunAgent = NonNullable<Awaited<ReturnType<AlphacodeClient["app"]["agents"]>>["data"]>[number]

type RunResourceMap = NonNullable<Awaited<ReturnType<AlphacodeClient["experimental"]["resource"]["list"]>>["data"]>
...
export type PermissionReply = Parameters<AlphacodeClient["permission"]["reply"]>[0]

export type QuestionReply = Parameters<AlphacodeClient["question"]["reply"]>[0]

export type QuestionReject = Parameters<AlphacodeClient["question"]["reject"]>[0]
